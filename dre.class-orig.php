<?php 
	class Gerar_DRE{
		...
		function getAgrupamentoDespesas($string){
			...
			if( $string == 'Condomínio' )
				return "despesas_administrativas";
			if( $string == 'Contador' )
				return "despesas_administrativas";
			if( $string == 'Correios' )
				return "despesas_administrativas";
			if( $string == 'Cursos e treinamentos' )
				return "despesas_administrativas";
			// if( $string == 'Empréstimo (amortização)' )
			// 	return "despesas_financeiras";
			// if( $string == 'Empréstimos' )
			// 	return "despesas_administrativas";
			if( $string == 'Energia elétrica' )
				return "despesas_administrativas";
			if( $string == 'Equipamentos' )
				return "despesas_administrativas";
			if( $string == 'Estagiários' )
				return "despesas_com_pessoal";
			if( $string == 'Impostos e encargos' )
				return "deducoes_com_impostos_devolucoes_e_descontos";
			if( $string == 'Internet' )
				return "despesas_administrativas";
			if( $string == 'Juros' )
				return "despesas_financeiras";
			if( $string == 'Licença de Software' )
				return "despesas_administrativas";
			if( $string == 'Limpeza' )
				return "despesas_administrativas";
			if( $string == 'Manutenção de equipamentos' )
				return "despesas_administrativas";
			if( $string == 'Manutenção de Veículos' )
				return "despesas_administrativas";
			if( $string == 'Marketing e publicidade' )
				return "despesas_de_vendas";
			if( $string == 'Material de escritório' )
				return "despesas_administrativas";
			if( $string == 'Outros' )
				return "outras_despesas";
			...
			if( $string == 'Pró-Labore' )
				return "despesas_com_pessoal";
			if( $string == 'Reembolso de despesas' )
				return "despesas_com_pessoal";
			if( $string == 'Segurança' )
				return "despesas_administrativas";
			if( $string == 'Seguros' )
				return "despesas_administrativas";
			if( $string == 'Taxas e Comissões Bancárias' )
				return "outras_despesas";
			if( $string == 'Telefone' )
				return "despesas_administrativas";
			if( $string == 'Transportadora / Motoboy' )
				return "despesas_administrativas";
			if( $string == 'Vale-Refeição' )
				return "despesas_administrativas";
			if( $string == 'Vale-Transporte' )
				return "despesas_administrativas";
			if( $string == 'Viagens e deslocamentos' )	
				return "despesas_administrativas";
		...
		function getAgrupamentoReceitas($string){
			...
			if( $string == 'Rendimentos de aplicação' )
				return "receitas_financeiras";
			if( $string == 'Serviços prestados em geral' )
				return "vendas_de_servicos";
			return false;
		}
				...
				<table border="0" cellspacing="2" cellpadding="4" style="width:840px;font-size: 11px;margin-top: -2px;float:left;margin-bottom:30px;">
					<tbody>
			    		...
			    		<tr>
							...
							<td class="td_calendario agrupado titulo_tabela">'.$this->getvendas_de_mercadorias_produtos_e_servicos().'</td>
						</tr>
						<tr>
				...
				<table border="0" cellspacing="2" cellpadding="4" style="width:840px;font-size: 11px;margin-top: -2px;float:left;margin-bottom:30px;">
					<tbody>
						<tr>
							...
							<td style="width:180px;" class="td_calendario agrupado titulo_tabela">'.$this->getcusto_das_vendas().'</td>
						</tr>
						<tr>
