<?php
/**
 * Classe criada para para manipularos dados da conta a pagar.
 * Data: 06/04/2018
 */
...
class ContasAPagarController {
	...
	function __construct($empresaId = false){
		...
		if($empresaId) {
			...
			$rs_ultimaData = $livroCaixa->pegaDadosContasAPagarUltimo($empresaId);

			// Função para pegar o primeiro pagamento. Localizado DataBaseMySQL/LivroCaixa.php
			$rs_primeiraData = $livroCaixa->pegaDadosContasAPagarPrimeiro($empresaId);
	...
	public function Montafiltro() {
		...
		$filtro = "<div style='float:left'><form action='/contas_a_pagar.php' method='get'>"
			." Mês: ".$selectMes
			." Ano: ".$selectAno
	...
	public function MontaTabela($empresaId) {
		...
		$table = "<table width='100%' cellpadding='5' style='margin-bottom:25px;'> "
				."		<tr> "
				."			<th width='30' style='font-size: 12px;'>Ação</th> "
	...
	private function PegaLinhasTabela($empresaId) {
		...
		// Método para pegar a lista de contas a pagar. Localizado DataBaseMySQL/LivroCaixa.php
		$dados = $livroCaixa->PegaDadosContasAPagar($empresaId, $selectAno, $selectMes);				
		...
		if($dados) {
			...
			foreach($dados as $val){
				...
				$out .= " <tr> "
						." <td class='td_calendario' align='center'><a id='excluirLinha_".$countAux."' style='cursor:pointer;' onclick='excluirLinha(".$empresaId.",".$val['id'].",".$countAux.")'><i class='fa fa-trash-o iconesAzul iconesGrd'></i></a></td> "	
						...
						." <td class='td_calendario'>".$this->VerificaDescricao($val['categoria'], $val['descricao'],$empresaId)."</td> "
						." <td class='td_calendario' align='right'>R$".number_format($val['saida'], 2, ',', '.')."</td> "
	...
	// Método criado para normalizar a descrição. 
	private function VerificaDescricao($categoria, $descricao, $empresaId){
		
		$out = $descricao;	
		
		$livroCaixa = new LivroCaixa();
		
		// Verifica se é um Pagamento e de acordo com id informado da descrição retorna o nome da pessoa.
		switch($categoria){
				
			case 'Estagiários':
				// Pega os dados do stagiario de acordo com o seu ID.
				$dados = $livroCaixa->PegaDadosEstagiarios($descricao, $empresaId);	
				$out = $dados['nome'];
				break;
				
			case 'Pagto. de Salários':
				// Pega os dados do funcionário de acordo com o seu ID.
				$dados = $livroCaixa->PegaDadosFuncionario($descricao, $empresaId);
				$out = $dados['nome'];
				break;
				
			case 'Pgto. a autônomos e fornecedores':
				// Pega os dados do fornecedor ou do autônomos de acordo com o seu ID.
				$dados = $livroCaixa->PegaDadosAutonomos($descricao, $empresaId);	
				$out = $dados['nome'];
				break;
				
			case 'Pró-Labore':
				// Pega os dados do sócio de acordo com o seu ID.
				$dados = $livroCaixa->PegaDadosSocio($descricao, $empresaId);	
				$out = $dados['nome'];
				break;			
	...
	public function IncluiContasAPagar($empresaId, $data, $saida, $documento, $descricao, $categoria, $vencimento) {
		
		$livroCaixa = new LivroCaixa();
		
		//Chama o metodo para realizar a inclusão do lançãmento de conta a pagar no livro caixa
		$livroCaixaId = $livroCaixa->InclusaoNoLivroCaixa($empresaId, $data, 0, $saida, $documento, $descricao, $categoria);		
		
		//Pega o id da ultima linha da tabela do livro caixa
		if($livroCaixaId){			
			
			// Define a categoria secundaria para normalizar o plano de contas do livro diário.
			$categoria_secundaria = $categoria." a pagar";
			
			//Chama o metodo para realizar a inclusão do lançamento de conta a pagar na tabela de relacionamento com o pagamento.
			$livroCaixa->lancamentoContasPagar($empresaId, $livroCaixaId, $categoria_secundaria, $vencimento);
			...
			// Redireciona para a tela de contas a pagar de acordo com a data da nota
			header('Location: /contas_a_pagar.php?periodoMes='.$mes.'&periodoAno='.$ano);
			
		}
		
		// Redireciona para a tela de contas a pagar 
		...
		foreach ($_FILES as $__FILES) {
			for ($i=0; $i < count($__FILES["name"]); $i++) { 
				...
				if($size_file > 1000000){
					...
					header('Location: contas_a_pagar.php?erro_file="'.$__FILES["name"][$i].'"');
					exit();
				}
			}
		}
