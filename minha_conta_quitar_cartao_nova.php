<?
	...
	// PEGA DESCONTO, PLANO E VERIFICA SE E PRÊMIO PARA PEGAR O CODIGO CONTADOR. 
	$sql_plano     = " SELECT plano
						, tipo_plano
						, contadorId
						, desconto_mesalidade
						, desconto_S_mensal
						, desconto_S_trimestral
						, desconto_S_semestral
						, desconto_S_anual
						, desconto_P_mensal
						, desconto_P_trimestral
						, desconto_P_semestral
						, desconto_P_anual
					FROM dados_cobranca 
					WHERE id = '" . $linha_meus_dados["id"] . "'";
	...
	$descontoMesalidade = $linha_plano['desconto_mesalidade'];

	$Config      = new Config(); //OBJETO DE CONFIGURAÇÃO
	$plano_meses = $Config->verMeses($plano_usuario); //RECUPERA MESES DO PLANO
	//$plano_valor = $Config->verValor($plano_usuario, $tipo_Plano, $descontoMesalidade); // RECUPERA VALOR DO PLANO


	// Pega o volor do plano e define qual devera ser o desconto.
	if($descontoMesalidade == 1){
		$plano_valor = $Config->verValor($plano_usuario, $tipo_Plano, $descontoMesalidade); // RECUPERA O VALOR DO PLANO
	} // Aplica o desconto individual. 
	elseif($descontoMesalidade == 2) {

		// Pega o valor do plano
		$plano_valor = $Config->verValor($plano_usuario, $tipo_Plano);

		// Verifica o tipo do plano para aplica o desconto.
		switch($plano_usuario){
			case 'mensalidade':
				if($tipo_Plano == 'P') {
					$plano_valor = $plano_valor - $linha_plano['desconto_P_mensal'];
				} else {
					$plano_valor = $plano_valor - $linha_plano['desconto_S_mensal'];
				}
				break;
			case 'trimestral':  
				if($tipo_Plano == 'P') {
					$plano_valor = $plano_valor - $linha_plano['desconto_P_trimestral'];
				} else {
					$plano_valor = $plano_valor - $linha_plano['desconto_S_trimestral'];
				}	
				break;
			case 'semestral':   
				if($tipo_Plano == 'P') {
					$plano_valor = $plano_valor - $linha_plano['desconto_P_semestral'];
				} else {
					$plano_valor = $plano_valor - $linha_plano['desconto_S_semestral'];
				}
				break;
			case 'anual':
				if($tipo_Plano == 'P') {
					$plano_valor = $plano_valor - $linha_plano['desconto_P_anual'];
				} else {
					$plano_valor = $plano_valor - $linha_plano['desconto_S_anual'];
				}
				break;
		}
	...
	*/	
	...
	if ( isset ( $_GET['atrasados'] ) && ( $linha_meus_dados['status'] == 'ativo' || $linha_meus_dados['status'] == 'inativo')   ) {
		...
		// Verifica se data informada e referente a um dia util.
		while(!$datas->ifDiaUtil($data_juros)) {
			$data_juros  = $datas->somarData($data_juros,1);
		}			
	...
	if ( $linha_meus_dados['id'] == 9 || $linha_meus_dados['id'] == 1581 ){
	...
	if (($retorno_codigo_erro == '') && ($retorno_codigo_autorizacao != '5')) {
	    ...
	    if (is_null($retorno_tid) || $retorno_tid == "") {
	        ...
	        $sqlup = "INSERT INTO relatorio_cobranca (id,idHistorico ,data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, tipo) VALUES ('" . $linha_meus_dados["id"] . "', '".$idHistorico."' ,'" . date('Y-m-d') . "', '" . $forma_pagamento_assinante . "', '2.4', 'não enviado', '', " . $valor_pago . ", '" . $numeroCartao . "', '".$tipo_Plano."', '".$plano_usuario."', '".$nomePlano."')";
			...
			if($tipo_Plano == 'P') {
				...
				$qryUpdate = "INSERT INTO `cobranca_contador` (`idRelatorio`,`contadorId`,`valor_total`,`valor_liquido`,`desconto_porcentagem`
) VALUE (".$idRelatorio.", ".$contadorId.", ".$valorTotal.", ".$valorLiquido.", ".$desconto.");";
	        ...
	        // return;
	        // INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
	        $sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico , data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, tipo) VALUES ('" . $linha_meus_dados['id'] . "', '".$idHistorico."' , '" . date('Y-m-d') . "', '" . $forma_pagamento_assinante . "', '2.1', 'enviado', '" . $retorno_tid . "', " . $valor_pago . ",'" . $numeroCartao . "', '".$tipo_Plano."', '".$plano_usuario."', '".$nomePlano."')";
