...
//	
	...
	class Boleto{
		...
		function getDadosUser(){
			...
			if( $this->gettipo_boleto() == 'mensalidade' )
				$this->setvalor_boleto($this->getValorPlano());
		...
		function getValorPlano(){
			$consulta = mysql_query("SELECT * FROM configuracoes WHERE configuracao = '".$this->getplano_user()."' AND tipo_plano = 'S' ");
			$objeto=mysql_fetch_array($consulta);
			return $objeto['valor'];
		}
		...
		function recalcularBoleto(){
			...
			echo '</div></div>';
			echo '<br><br>';
			echo '<iframe src="https://www63.bb.com.br/portalbb/boleto/boletos/hc21e,802,3322,10343.bbx" style="overflow:hidden;width: 1280px;margin: 0 auto;text-align: left;height: 1600px;border: 0;margin-bottom: 20px"></iframe>';
		}
		...
		function verificaSegundaVia(){
			...
			if( isset($objeto['numdoc_get']) && $objeto['numdoc_get'] == $this->getnosso_numero() ){
				...
				if( intval($objeto['valor_get']) != intval($this->getvalor_boleto()) && $objeto['remessa_gerada'] == 0 ){
					...
					if( $this->getdata_vencimento() < date("Y-m-d") )
		...
		function registrarBoleto(){	
			$campos = "(`id_user`, `id_historico`, `nosso_numero`, `status`, `tipo`, `plano`, `valor`, `multa`, `mora`, `vencimento`)";
			$values = "(
				'".$this->getid_user()."',
