	...
	class Boleto{
		...
		function registrarBoleto(){	
			...
			if($this->getContadorId() && $this->gettipo_boleto() == 'AbertAltEmpresa' || $this->gettipo_boleto() == 'AbertAltSociedade' || $this->gettipo_boleto() == 'decore' || $this->gettipo_boleto() == 'DBE') {
				...
				$qry = "INSERT INTO `cobranca_contador` (`contadorId`, `boletosRegistradosId`) VALUE (".$this->getContadorId().", ".$boletosRegistradosId.");";
				
				echo $qry;
				
				mysql_query($qry);
			} else if($this->getContadorId() && $this->getTipoPlano() == 'P' && gettipo_boleto() == 'mensalidade') {
