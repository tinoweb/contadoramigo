<?php 
	...
	class Minha_conta{
		...
		function getFraseNotificacaoErro(){
			$string = '
				<div style="clear: both; margin-top: 15px;">
			        <span class="tituloVermelho">O pagamento não pode ser realizado, pois a sua operadora não autorizou a transação.</span><br>
			        <br>
			    </div>
			';
			return $string;
		...
		function getFraseNotificacaoErroInvalido(){
			$string = '
				<div style="clear: both; margin-top: 15px;">
			        <span class="tituloVermelho">O pagamento não pode ser realizado, pois o cartão informado é inválido.</span><br>
			        <br>
			    </div>
			';
			return $string;
		...
		function getConfiguracoes(){
			$consulta = mysql_query("SELECT * FROM configuracoes WHERE tipo_plano = 'S'");
			return $consulta;
		}
		function exibirNomePlano($item){
			if( $item['configuracao'] == 'mensalidade' ){
				return 'Mensal: '.$this->tomoney($item['valor']);
			}
			if( $item['configuracao'] == 'trimestral' ){
				return 'Trimestral: '.$this->tomoney($item['valor']);	
			}
			if( $item['configuracao'] == 'semestral' ){
				return 'Semestral: '.$this->tomoney($item['valor']);
			}
			if( $item['configuracao'] == 'anual' ){
				return 'Anual: '.$this->tomoney($item['valor']).' (R$ 49,00 por mês)';
			}
		}
		function getPlanospagamento(){
			...
			$itens = $this->getConfiguracoes();
			while( $item = mysql_fetch_array($itens) ){
				
				$string .= '
						<tr>
							<td>
								<input type="radio" class="atualizarViaAjaxText atualizarValorAvencer" tabela="dados_cobranca" campo="plano" name="radPlano" id="radPlano_'.$item['configuracao'].'" value="'.$item['configuracao'].'" '.$this->checked($item['configuracao'],$this->getplano()).'>
							</td>
							<td>
								<label for="radPlano_mensalidade">'.$this->exibirNomePlano($item).'</label>
								<input type="hidden" id="id_'.$item["configuracao"].'" value="'.$item["valor"].'" />
							</td>
						</tr>
				';
			}
		...
		function getPlanoUsuario(){
			if( $this->getplano() == 'mensalidade' )
				return "Mensal";
			if( $this->getplano() == 'trimestral' )
				return "Trimestral";
			if( $this->getplano() == 'semestral' )
				return "Semestral";
			if( $this->getplano() == 'anual' )
				return "Anual";
		}
		...
		function getValorMensalidade(){
			$planos = array();
			$consulta = mysql_query("SELECT * FROM configuracoes WHERE tipo_plano = 'S'");//Busca os planos cadastrados no banco
		...
		function calcularMensalidade($id){
			...
			if( $objeto_consulta['valor'] != 0 )
				...
				return floatval( $this->getValorMensalidade() * $this->gettotal_empresas() );
		}
		...
		function getTextoDemoInativo(){
        	...
        	if( $this->ifDadosCobrancaPreenchidos() ){
        		$string .= ', no valor de <span class="destaque valorProximoAVencer">'.$this->tomoney($this->getValorMensalidade()).'</span>';
        	}
        	$string .='.<br><br />';
        	echo $string;
		}
		...
		function getTextoDemo(){
			$string = '
	            ...
	            <br />
	            Você poderá utilizar o Contador Amigo por mais <span class="destaque">'.$this->diasRestantesDemo().'</span> dias.<br />
	            <br />
        	...
        	if( $this->ifDadosCobrancaPreenchidos() ){
        		$string .= ', no valor de <span class="destaque valorProximoAVencer">R$ '.$this->tomoney($this->getValorMensalidade()).'</span>';
        	}
        	$string .='.<br><br />';
        	echo $string;
		}
		...
		function getTextoCancelado(){
			$string = '
		        ...
		        <br /> 
		       	...
		       	<span class="destaque">'.$this->tomoney($this->getValorMensalidade()).'</span> Seus débitos anteriores serão perdoados. <br /><br /> 
			...
			while( $item = mysql_fetch_array($faturas_pagas) ){
				...
				$string .= '
						<tr class="guiaTabela" style="background-color: rgb(255, 255, 255);" valign="center">
							...
							<td align="center">    
					        	'.$this->getPlanoPorValor($item['valor_pago']).'
							</td>
							<td align="center">    
		...
		function getPlanoPorValor($plano){
			$plano = floatval($plano);
			if( $plano <= 100 )
				return 'Mensal';	
			if( $plano < 177 && $plano % 59 == 0 )
				return 'Mensal';		
			if( $plano > 59 && $plano < 354)
				return 'Trimestral';		
			if( $plano >= 354 && $plano < 500 )
				return 'Semestral';
			if( $plano > 500 )
				return 'Anual';
			return 'Mensal';
		...
		function getFaturasEmAberto(){
			...
			while( $item = mysql_fetch_array($faturas_pendentes) ){
				...
				$string .= '
						<tr class="guiaTabela" style="background-color: rgb(255, 255, 255);" valign="center" id="'.$item['idHistorico'].'" data="'.$item['data_pagamento'].'" boleto-vencido="'.$this->verificarBoletoVencido($item['idHistorico']).'" via="'.$this->verificarViaBoleto($item['idHistorico']).'" gerar-segunda-via="'.$this->verificarStatusBoleto($item['idHistorico']).'">
					        ...
					        <td align="center" class="valorProximoAVencer">
					        	'.$this->toMoney($this->calcularMensalidade($item['idHistorico'])).'
							</td>
		...
		function getSqlFaturasPagas(){
			$consulta = mysql_query("SELECT * FROM relatorio_cobranca WHERE id = '".$this->getid_user()."' AND ( resultado_acao = '1.2' OR resultado_acao = '2.1' ) ORDER BY idRelatorio DESC LIMIT 5");
			return $consulta;
		}
		...
		//Funcao que pega os dados de login do usuario
