<?
	...
	//pega dados da tabela empresa
		...
		function __construct(){
			...
			$this->setvalor($objeto['valor']+$this->calcularIOF($objeto));
	...
	$pdf->SetFont('arial','',10);
	$pdf->MultiCell(170,5,utf8_decode("CLÁUSULA PRIMEIRA: ".ucfirst($gambiarras_mutuante)." empresta ".$gambiarras_mutuario." a quantia de R$ ".number_format($dados->getvalor(),2,',','.')." (".$valor_extenso."), que deverá ser quitada até o dia ".$dados->getdata_devolucao().", acrescida de juros de ".number_format($dados->getjuros(),2,',','.')."% (".$porcentagem_extenso.")."),0,'J');
