...
<head>
<meta id="view_port" name="viewport" content="">

<?php include 'meta_'.$nome_meta.'.php'; ?>

<link id="teste_css" href="estilo.css" rel="stylesheet" type="text/css" />
...
<script>
	...
	<?php 
		...
		if( $_SERVER['REQUEST_URI'] == '/index.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/quem_somos.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/saiba_mais.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/duvidas_frequentes.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/contato_mobile.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/assinatura.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/abertura_empresa.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/enquadramento_simples.php' )
			$mobile = true;
		if( $_SERVER['REQUEST_URI'] == '/descadastramento_mei.php' )
			$mobile = true;
	...
	if(w < 800){
		$("#teste_css").attr("href","estilo_mobile.css");
		$("#view_port").attr("content","width=device-width, initial-scale=1.0");
		$(document).ready(function() {
			$(".conteudo_desktop").empty();
			$(".rodape_desktop").empty();
		});
	}
	else{
		$(document).ready(function() {
...
<div class="conteudo_desktop">

<div class="principal" style="position: relative; margin-bottom:20px;">

<div style="float:left; width:396px; height:73px">
<a href="<?=$dominio?>index.php"><img src="images/logo.png" alt="Contador Amigo" title="Contador Amigo" width="396" height="68" border="0" style="margin-bottom:5px; float:left"></a></div><div style="float:left; margin-top:45px; font-family:vagLight; width:100px; color:#024a68; font-size:11px">Desde 2012</div><div style="float:left; margin-left:140px; margin-top:20px"><a href="http://www.desenvolvesp.com.br/" target="_blank" border="0"><img src="images/desenvolve_sp.png" alt="Apoio: Desenvolve SP" width="80" title="Desenvolve SP"/></a></div>

<div style="float:right; margin-top:40px; text-align:right; font-size:24px">
<a href="https://www.youtube.com/c/ContadoramigoBrasil" alt="Canal Contador Amigo no Youtube" title="Canal Contador Amigo no Youtube" target="_blank"><i class="fa fa-youtube-square" style="color:#003d62"></i></a>
...
<!--BALLOM CONTATO -->
<div style="width:365px; position:absolute; margin-top:15px; margin-left:550px; display:none; z-index:10" id="contato" class="bubble_top box_visualizacao x_visualizacao">
...
<div class="titulo" style="margin-bottom:10px">Contato</div>
  <form action="contato_envio.php" method="post" name="form" id="form" style="display:inline">
  	<input type="hidden" name="id_contato" value="" placeholder="Este campo deve ficar vazio">
  	<div style="width: 100%;">
    ...
    <input type="hidden" name="paginaAtual" id="paginaAtual" value="<?=substr($_SERVER['REQUEST_URI'],1)?>" />
...
<?php 
	...
	//INICIO - Alteração dia 09/05/2016
	
	//Manter o ballon do contato quando exibir a mensagem de confirmação
 ?>

<?php if(isset($_GET["email_enviado"])) { ?>
<div style="width: 365px; position: absolute; margin-top: 15px; margin-left: 550px; display: block; z-index: 10;" id="contato2" class="bubble_top box_visualizacao x_visualizacao"><div style="position: absolute; top: 10px; right: 10px; display: block;"><img src="images/x.png" style="width: 8px; height: 9px; border: 0px; cursor: pointer;"></div>

<div style="padding:20px;">

<div class="titulo" style="margin-bottom:10px">Contato</div>
<div height="25" style="padding-top:0px; text-align:left; font-size:14px">Mensagem enviada com sucesso!<br>
Em breve faremos contato.</div>
...
<script>
	...
	$("#contato2").css("display","block");

	$("#contato2 img").click(function() {
		$("#contato2").css("display","none");
	});
