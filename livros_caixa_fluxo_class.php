...
{        
    ...
    public function listarCategorias($tipo, $ano, $categoriaEmprestimo)
    {
		$this->result = mysql_query(
			...
			WHERE (" . $tipo . " > 0) AND (YEAR(data) = " . $ano . ") AND (Categoria NOT IN ('" . $categoriaEmprestimo . "'))
			GROUP BY categoria"
		)
    ...
    public function listarCategorias2($tipo, $ano, $categoriaEmprestimo)
    {
		$this->result = mysql_query(
			"SELECT categoria
			FROM user_" . $this->id . "_livro_caixa
    ...
    public function listarValores($tipo, $ano, $categoriaInclude, $categoriaExclude)
    {
		...
		if ($categoriaInclude != "")
		{
			$sqlFilter = $sqlFilter . " AND (Categoria IN ('" .  $categoriaInclude . "'))";			
		}			
		
		if ($categoriaExclude != "")
		{
			$sqlFilter = $sqlFilter . " AND (Categoria NOT IN ('" .  $categoriaExclude . "'))";			
		}
    ...
    public function totalCategoria($tipo, $ano, $categorias)
    {
    	$result = mysql_query(
			"SELECT SUM(" . $tipo . ") as total 
			FROM user_" . $this->id . "_livro_caixa 
			WHERE (YEAR(data) = " . $ano . ") AND (categoria IN ('" . $categorias . "'))"
		)or die (mysql_error());
