...
<div class="principal minHeight">		
	...
	<div class="hideImpressao">
		<p><strong>Pellentesque habitant morbi tristique</strong> senectus et netus et malesuada fames ac turpis egestas. Vestibulum tortor quam, 
			feugiat vitae, ultricies eget, tempor sit amet, ante. Donec eu libero sit amet quam egestas semper. <em>Aenean ultricies mi vitae est.
		</em> Mauris placerat eleifend leo. Quisque sit amet est et sapien ullamcorper pharetra. Vestibulum erat wisi, condimentum sed, <code>commodo 
		vitae</code>, ornare sit amet, wisi. Aenean fermentum, elit eget tincidunt condimentum, eros ipsum rutrum orci, sagittis tempus lacus enim ac 
		dui. In turpis pulvinar facilisis. Ut felis.</p>
				
		<br>
	</div>
	<form action="balanco.php" method="GET" accept-charset="utf-8">
		<?php 
				...
				for ($i=intval(date("Y")); $i > intval(date("Y")) - 5; $i--) { 
					...
					echo '	<label style="margin-right:20px;">
								<input style="margin-right:5px;" type="radio" name="ano" value="'.$ano_atual.'"';
		...
		<input type="hidden" name="gerar-dre" value="">

		<input id="exibir" type="submit" value="Exibir" style="margin-left:20px;">
	</form>
	<br>
	...
	<?php 
		...
		$consulta_dados_empresa = mysql_query("SELECT * FROM dados_da_empresa WHERE id = '".$_SESSION['id_empresaSecao']."' ");
		$objeto_dados_empresa=mysql_fetch_array($consulta_dados_empresa);

		echo '<div class="pagina">';
		echo '<div class="tituloVermelho apenasImpressao" style="font-size: 20px;"><strong>'.strtoupper($objeto_dados_empresa['nome_fantasia']).'&nbsp;&nbsp;&nbsp;CPNJ: '.$objeto_dados_empresa['cnpj'].'</strong></div><br>';
		echo '<div class="tituloVermelho">Demonstração do Resultado do Exercício de '.$ano.' </div><br>';
		echo '<div class="tituloVermelho apenasImpressao"><strong>Demonstração do Resultado do Exercício de '.$ano.' </strong></div><br>';
		if( isset( $_GET['gerar-dre'] ) ):

			$resultado_dre = executeDRE($_GET['ano']);
			
		else:

			$resultado_dre = executeDRE(date("Y"));

		endif;
		echo '<div style="width:966px;"><br><center><button id="btnCSV1">Exportar para Excel</button></center></div>';
		echo '</div>';
	 ?>
	 <br><br>
	 <?php 
		...
		echo '<div style="width:966px;"><br><center><button id="btnCSV2">Exportar para Excel</button></center></div>';
		echo '</div>';
		...
		echo '<div style="width:966px;"><br><center><button style="margin-right:20px;"class="btnPRINT">Imprimir Balanço</button></center></div>';
 	
 		echo '<br><br><br><br><br><br><br><br>';
 		echo '<center></center>';
 		echo '<div class="tituloVermelho apenasImpressao" style="text-align:center;width:40%;float:left"><p>________________________________________</p>'.$nome_responsavel.'<br>CPF: '.$cpf_responsavel.'<br>Responsável Legal '.$nome_empresa.'</div>';
		echo '<div class="tituloVermelho apenasImpressao" style="text-align:center;width:40%;float:right"><p>________________________________________</p><span class="impressao_contador_nome"></span><br><span class="tipo_contador2">CRC</span>: <span class="impressao_contador_crc"></span><br>Contador Responsável '.$nome_empresa.'</div>';
		echo '</div>';
		echo '<div style="clear: both;height: 10px;"></div>';
 ...
 ?>
	...
	<style type="text/css" media="screen">
		.a_n_c_intangivel,.a_n_c_imobilizado,.a_n_c_depreciacao,.p_l_lucros_acumulados,.p_l_prejuizos_acumulados{
			border:0;
			background: #e5e5e5;
		}
		.agrupado{
	...
	function salvarDadosContador(tipo,nome,crc,endereco,cidade,estado,cep,id_item){
		...
		$(".btnPRINT").click(function() {
			...
			var ano_aux = $(".ano_tabela").val();
			var nome_arquivo = '<?php echo 'Balanço - '; ?>'+ano_aux;
...
<script>
	...
	$(".fecharDiv").click(function() {
		...
		if( $(this).attr("abrir") === 'imobilizados' ){
	        window.location.replace(url + "#imobilizados");
	    }

	    if( $(this).attr("abrir") === 'intangiveis' ){
	        window.location.replace(url + "#intangiveis");
	    }
	...
	$( document ).ready(function() {
	    ...
	    var resultado_dre = $(".total_dre").html();

	    var result = resultado_dre.replace(".", "");
		var result = result.replace(",", ".");

	    aux = parseFloat(result);
	    if( aux > 0 )
	    	campo = "p_l_lucros_acumulados";
	    else
	    	campo = "p_l_prejuizos_acumulados";
