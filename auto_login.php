<?php 
	...
	// INICIO - Alterações do dia 03/05/2016
	// Classe que filtra o POST visando evitar SQL injection
...
// Pega o Dominio utilizando server. 
...
//	setcookie('contadoramigoHTTPS', $email . " " . $senha, time()+(120*120*24*30), '/', 'contadoramigo.com.br', 0);	
	setcookie('contadoramigoHTTPS', $email . " " . $senha, time()+(120*120*24*30), '/', str_replace('www.', '', $_SERVER['SERVER_NAME']), 0);	
	//session_start();
	//session_destroy();
...
if(isset($_REQUEST["login"])){
	...
	//MAL Em caso de tentar logar sem cookie e com session aberta no site e com dados preenchidos, mata a sessão e cria uma nova
	if( isset( $_SESSION['id_userSecao'] ) && $_SESSION['id_userSecao'] != '' ){
		
		$dadosContador = false;
		$adminArray = false;
		
		// Verifica se existe a sessão do Area do contador.
		if(isset($_SESSION['DadosContador'])) {
			$dadosContador = $_SESSION['DadosContador'];	
		}
		
		// Verifica se sera necessário recriar a sessão do admin.
		if( isset($_SESSION['nomeAdm_userSecao']) && isset($_SESSION['emailAdm_userSecao']) && isset($_SESSION['idAdm_userSecao']) && isset($_SESSION['IsAdmAuthorized']) && isset($_SESSION['idAdmSecao']) ){
			
			$adminArray = array('nomeAdm_userSecao'=>$_SESSION['nomeAdm_userSecao'],
							'emailAdm_userSecao'=>$_SESSION['emailAdm_userSecao'],
							'idAdm_userSecao'=>$_SESSION['idAdm_userSecao'],
							'IsAdmAuthorized'=>$_SESSION['IsAdmAuthorized'],
							'idAdmSecao'=>$_SESSION['idAdmSecao']);				
		}
		
		// Destroi a Sessão anterior.
		session_destroy();
	
		// Inicia uma nova sessão.
		session_start();
		
		// Recria a sessão da area do contador.
		if($dadosContador){
			$_SESSION['DadosContador'] = $dadosContador;	
		}

		// Recria a sessão da area do admin.
		if($adminArray){
			
			// Loop para criar a sessão.
...
//		setcookie('contadoramigoHTTPS', $_POST['txtEmail'] . " " . md5($_POST['txtSenha']), time()+(120*120*24*30), '/', 'contadoramigo.com.br', 0);
		setcookie('contadoramigoHTTPS', $_POST['txtEmail'] . " " . md5($_POST['txtSenha']), time()+(120*120*24*30), '/', str_replace('www.', '', $_SERVER['SERVER_NAME']), 0);
	...
	if (!$recordSet){ //Condição: Caso há problemas na consulta com o banco de dados...
		...
		$_SESSION['mensERRO'] = "Usuário não encontrado." . mysql_error(); //Mensagem de erro alertando que não foi encontrado nenhum usuário, não está sendo utilizada no momento, ao invés disso o erro será enviado através do campo "erro" pelo link abaixo.
...
//		$qtdEmpresas							=	mysql_num_rows($rsCheckQtdEmpresas); //Extrai o conteúdo amarzenado no banco de dados
		...
		if (($_SESSION['status_userSecao']=='inativo') or ($_SESSION['status_userSecao']=='demoInativo') or ($_SESSION['status_userSecao']=='cancelado')) {
			$URL = $_SESSION['url'];
			if( $URL == 'promo_certificado.php' ){
				// $URL = $_SESSION['url'];
				unset($_SESSION['url']); //Desativa a sessão após ser utilizada.
				header('Location: '.$dominio_seguro.$URL);
				exit;
			}
			if( $URL == 'boleto_promocao.php' ){
				// $URL = $_SESSION['url'];
				unset($_SESSION['url']); //Desativa a sessão após ser utilizada.
				header('Location: '.$dominio_seguro.'boleto_promocao.php');
				exit;
			}
