...
jQuery(document).ready(function() {		
	...
	$('#btSalvar').click(function(){
		...
		var apelidoAntigo = $("#ApelidoAntigo").val();
		...
		if($campoDocumento.val() != ''){
			$.ajax({
				url:'meus_dados_clientes_checa.php?id=' + $('#hidID').val() + '&idLogin=' + $('#hidID2').val() + '&documento=' + $campoDocumento.val() + '&tipo=' + $txtTipoDocumento + '&apelido='+apelido + '&apelidoAntigo='+apelidoAntigo,
				type: 'get',
				async: 	true,
				cache: false,
...
if($_GET['act'] == 'new' || $_GET["editar"]){ ?>
  ...
  <tr>
    ...
    <td class="formTabela" colspan="3">
    	...
    	<input name="ApelidoAntigo" id="ApelidoAntigo" type="hidden" value="<?=$apelido?>" alt="Apelido" />
    </td>
  </tr>
...
<div style="text-align: right; margin-bottom:10px; width:100%"><a href="meus_dados_clientes.php?act=new">Cadastrar novo cliente</a></div>
<?
