...
<?php 
	...
	// $consulta_empresas = mysql_query("SELECT * FROM dados_da_empresa ");
	// while( $objeto_consulta_empresas = mysql_fetch_array($consulta_empresas) ){
	// 	$consulta_livro_caixa = mysql_query("SELECT * FROM user_".$objeto_consulta_empresas['id']."_livro_caixa WHERE categoria LIKE '%outros%'  ");
	// 	$objeto_consulta_livro_caixa = mysql_fetch_array($consulta_livro_caixa);
	// 	if( $objeto_consulta_livro_caixa['id'] != '' ){
	// 		$consulta_login = mysql_query("SELECT * FROM login WHERE idUsuarioPai = '".$objeto_consulta_empresas['id']."' AND id = idUsuarioPai ");
	// 		$objeto_consulta_login = mysql_fetch_array($consulta_login);
	// 		echo $objeto_consulta_login['id'];
	// 		echo '<br>';
	// 	}
	// 	// <option value="Devolução de adiantamento">Devolução de adiantamento</option>
	// }
...
<div class="principal">
	...
	<h2 class="hideImpressao">Livro Diário</h2>
	<h2 class="apenasImpressao">Livro Diário - de <?php echo getAnoInicio(); ?> até <?php echo getAnoFim(); ?></h2>
	<h2 class="apenasImpressao"><?php echo $_SESSION['nome_userSecao']; ?></h2>
	<p class="hideImpressao">
		O livro diário registra as entradas e saídas da empresa no formato contábil. É usado na elaboração do balanço. Você não precisará atualizá-lo, pois ele é gerado automaticamente a partir do seu livro-caixa.
	</p>
	<br>
	...
	<script>
		$( document ).ready(function() {
			$("#alterarPeriodo").click(function() {
				...
				location = 'livro-diario.php?tipo=diario&DataInicio='+inicio+'&DataFim='+fim;
			});
		});
	</script>
...
//		$query = " SELECT c.* FROM user_".$_SESSION['id_empresaSecao']."_livro_caixa c"
...
//			." AND c.categoria != '' "
//			." AND  c.categoria != 'Repasse a terceiros' "
...
/*				echo '	<br><div class="tituloVermelho">Impossível Prosseguir</div>
						...
						incompatíveis com o Livro Diário. 
...
/*			,'Pagto. de Salários'
	...
	function NormalizaContasApagarEContasReceber($item){
		...
		if(isset($item['categoria_secundaria_2']) &&  !empty($item['categoria_secundaria_2'])){
			...
			$itemArray = array(
				...
				'categoria_secundaria_1'=>$item['categoria_secundaria_1'],
				'categoria_secundaria_2'=>$item['categoria_secundaria_2']
			); 
	...
	function MontaLinhaTabela($livro_diario, $item, $plano_contas ) {
		...
		// Preservar este Switch ate revisar o banco, pois este são categorias antigas. 
		switch($item['categoria']){

			case 'Pgto. de Salários':
				$categorias_livro = $livro_diario->getRelacao('Pagto. de Salários',$item);
				break;
			case 'Despesas bancárias':
				$categorias_livro = $livro_diario->getRelacao('Taxas e Comissões Bancárias',$item);
				break;
			case 'Licença de Software':
				$categorias_livro = $livro_diario->getRelacao('Aluguel de software',$item);
				break;
			case 'Licença ou aluguel de softwares':
				$categorias_livro = $livro_diario->getRelacao('Aluguel de software',$item);
				break;
			case 'Juros':
				$categorias_livro = $livro_diario->getRelacao('Empréstimo (amortização)',$item);
				break;
			default:
...
<center>
	<div style="width:966px;">
		<center>
			<button style="margin-right:20px;" onclick="window.print();">Imprimir Livro Diário</button>
			<a href="exportar-csv-livro-diario-razao.php?tipo=diario&DataInicio=<?php echo getAnoInicio(); ?>&DataFim=<?php echo getAnoFim(); ?>" target="_blank">
