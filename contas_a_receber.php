...
//	error_reporting(E_ALL);
	...
	if(isset($_POST['pesquisaAjax'])) {
		...
		echo json_encode(array('status'=>$retorno, 'id' => $idLinha));
	...
	if(isset($_POST['data'])) {
		...
		require_once('Controller/contas_a_receber-controller.php');
		...
		var_dump($valor, $docNum, $descricao, $empresaId, $categoria);
				
		// Chama o metodo que realiza ainclusão dos dados.
		$contasAReceber->IncluiContasAReceber($empresaId, $data, $valor, $docNum, $descricao, $categoria , $vencimento);	
	...
	if(isset($_POST['idLinha'])){
		...
		$contasAReceber->excluiDados($_POST['idLinha'], $_POST["idEmpresa"]);
	...
	// Instancia o controle da página.
	$contasAReceber = new ContasAReceberController($empresaId);
?>
<style>
...
<div class="principal">
	...
	<form id="form_contas" method="post" action="contas_a_receber.php" enctype="multipart/form-data">
		
		<div style="clear:both; margin-bottom: 10px;">
			<label>Categoria:</label>
			
			<?php echo $contasAReceber->pegaListaCliente($empresaId); ?>
	...
	<?php
		...
		echo $contasAReceber->MontaFiltro();
	
		// Apresenta a tabela. estes métodos estão em Controller/contas_a_receber-controller.php
		echo $contasAReceber->MontaTabela($empresaId);
	?>
	
	
</div>

<!--BALLOM excluir contas a receber-->
<div class="bubble only-box" style="display: none; padding:0; position:absolute; top: -50px; left:50%; margin-left: -450px; z-index: 9999;" id="aviso-delete-livro-caixa">
