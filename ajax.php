...
//	error_reporting(E_ALL);
	...
	if( isset( $_POST['selectPagamentos'] ) ):
		...
		if( $_POST['tabela'] == 'funcionarios' ){
			...
			if( mysql_num_rows($consulta) == 0 )
				$frase_erro = 'Clique <a href="meus_dados_funcionario.php" title="Cadastrar Funcionario" target="_blank">aqui</a> para cadastrar um funcionário';
			else
			while( $objeto_consulta = mysql_fetch_array($consulta) )
	...
	if( isset( $_POST['atualizarMinhaConta'] ) ):
		...
		else if( $tabela == 'dados_cobranca' && $campo == "tipo_plano" ){
			$tipo_plano = $_POST['valor'];
			mysql_query("UPDATE ".$tabela." SET ".$campo." = '".$tipo_plano."' WHERE id = '".$id_user."' ");
		}
		else
	...
	if( isset( $_POST['gpsCalcularJuros'] ) ):
		...
		// No primeiro mês o juros por atraso e de 1%. Nos meses seguintes é a selic.
		...
		$mes = $data_aux[1];
		$ano = $data_aux[0];
		$data_vencimento2 = $data_vencimento = $data_aux[0].'-'.$data_aux[1].'-20';
		$fim_de_semana = 0;
...
//		}
		...
		//Se dia 20 cai em feriado ou fim de semana, leva até o primeiro dia util anterior 
		while( !$datas->ifDiaUtil($data_vencimento2) ){
			$data_vencimento2 = $datas->subtrairData($data_vencimento2,1);
		}
		
		// Soma mais um dia para descobrir quando começa a contar a multa.
		$data_vencimento2 = $datas->somarData($data_vencimento2,1);
		...
		// Pega a diferença de dias em atraso.
		$dias = $datas->diferencaData($data2, $data_vencimento2);
		
		// O que é chamado de mora na realidade e a multa.
		$mora = 0;

		if( $dias > 0 ){	
			$juros = ($dias) * 0.0033;
			if( $juros > 0.2 ){
				$va1 = $mora = floatval($valor) * 0.2;
			}
			else{
				$va2 = $mora = $juros * floatval($valor);
			}
		}
		
		// Preserva 2 casas sem arredondar.
		$mora = DecimalSemArredondar($mora, 2);
		...
		$array[5] = "";
		$array[6] = 'Dias - '.$dias;
		$array[7] = 'vencimento - '.$data_vencimento2;
		$array[8] = 'juros - '.$juros;
		$array[9] = 'mes de pagamento - ';
		$array[10] = 'vencimento - '.$Vencimento;
		$array[11] = 'pagamento - '.$data2;
	...
	if( isset( $_POST['calcularIOF'] ) ):
		...
		// Devolve valor liquido com desconto da IOF. Átano 
		$array[7] = number_format( ($valor_liquido - $valorIOF) , 2 , ',' , '.'  );
	...
	if( isset( $_POST['calcularIOF2'] ) ):
		...
		$valor = floatval(str_replace(",",".",str_replace(".","",$_POST['valor'])));
		$data1 = $data->converterData($_POST['data1']);
		$prazo = intval($_POST['dias']);
		$juros = floatval(str_replace(",",".",$_POST['juros']))/100;

		$data2 = $data->somarData($data1,$prazo);
		...
		//Pega o valor total com juros
		//$valor = $valor * $juros + $valor;
		$valor = $valor  + ($valor * ($prazo * ( $juros / 30 )));
		...
		$data2aux = calcularDataVencimento($data2);
		...
		//Salva a data de pagamento do emprestimo
		$array[3] = $data->desconverterData($data2);
	...
	if( isset( $_POST['getDadosSocioResponsavel'] ) ):
		...
		$id = $_POST['id'];
		$consulta = mysql_query("SELECT * FROM dados_do_responsavel WHERE id='".$empresaId."' AND responsavel = 1 LIMIT 0, 1;");
	...
	function calcularValidadeJuros($dataInicio, $dataFim, $juros) {
			
		include 'datas.class.php';	
		
		$data = new Datas();

		$data_inicio = $data->converterData($dataInicio);
		$data_fim = $data->converterData($dataFim);
