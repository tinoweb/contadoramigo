...
Para que o contrato tenha validade, você deve estipular uma taxa de juros para o empréstimo. Nosso sistema sugere uma taxa de 1% ao mês. Você poderá alterar para mais, desde que não ultrapasse a Selic (que hoje está em <?php echo $taxa_selic; ?> ) ou para menos, desde que não fique muito abaixo da média do mercado, para não descaracterizar o empréstimo. </div>
		...
		<form id="formGerarReciboEmprestimo" action="recibo_emprestimo.php" method="post">
			...
			<!-- <form id="formGerarReciboEmprestimo" action="recibo_emprestimo.php" method="post"> -->
			<table width="90%" cellpadding="3" style="margin-bottom:10px;">			
			...
			<div style="margin-bottom:20px;margin-top:30px;position: relative;">
				...
				<div style="margin-bottom: 5px;width:100%;float:left">
                    ...
                    <input type="hidden" id="iof2" name="IOF" />
				...
				<div style="margin-bottom: 5px;width:100%;float:left">
					...
					<input type="text" id="IRRF" name="IRRF" class="currency" value="" size="15" disabled style="margin-right:10px;float: left;">
                    <input type="hidden" id="IRRF2" name="IRRF" />
				...
				<div style="display:none;margin-bottom: 5px;width:100%;float:left;margin-top: 15px;" class="valor_total_repassar">
                    ...
                    <input id="valor_liquido" type="hidden" name="valor_liquido" />
				</div>
				<div style="display:none;margin-bottom: 15px;width:100%;float:left" class="valor_a_restituir">
                    ...
                    <input id="valor_a_restituir" type="hidden" name="valor_a_restituir" />
				</div><br>
				...
				function realizacalculo(){
					...
					$.ajax({
						...
						, success: function(retorno){
							...
							$("#valor_liquido").val(obj[7]);
							$("#valor_a_restituir").val(obj[1]);
...
//					}
		...
		<h2>Como recolher os impostos</h2>
			
		<ul>
		  	<li>Gere a guia dos impostos clicando no item correspondente na tabela abaixo.</li>
		</ul>
		
		<h2 style="margin-top:25px">Contrato de empréstimos cadastrados</h2>
		...
		<br /><br />
		<table id="tabela_itens" width="100%" cellpadding="5">
          	<tbody>
				<tr>
					<th width="7%">Ação</th>
                    ...
                    <th width="10%">Valor do empréstimo</th>
                    <th width="8%">IOF</th>
                    <th width="10%">Valor líquido</th>
					<th width="5%">Juros (mês)</th>
					<th width="11%">Valor a restituir</th>
					<th width="8%">Data da restituição</th>
					<th width="8%">IRRF</th>
					<th width="8%" colspan="2">Impostos</th>
				...
				<?php 
					...
					function pegaSituacaoIOF($item) {
						
						$valor = "Isento";
						
						if($item['tipo'] == 2) {
							$valor = 'R$ '.number_format($item['IOF_a_ser_recolhido'],2,',','.');
						}
						
						return $valor;
					}
					
					function pegaValorLiquido($item) {
						
						$valor = $item['valor'];
						
						if($item['tipo'] == 2) {
							$valor = $valor - $item['IOF_a_ser_recolhido'];  
						}
                ...
                <tr>
                    ...
                    <td class="td_calendario"><?php echo ($item['tipo'] == 1 ? 'sócio para empresa' : 'empresa para sócio'); ?></td>
                    <!-- <td class="td_calendario" align="left"><?php echo $data->desconverterData($aux_data_emprestimo[0]); ?></td> -->
                    <td class="td_calendario" align="center"><?php echo date('d/m/Y', strtotime($item['data']));?></td>
                    <td class="td_calendario" align="right">R$ <?php echo number_format($item['valor'],2,',','.'); ?></td>
                    <td class="td_calendario" align="right"><?php echo pegaSituacaoIOF($item); ?></td>
                   	<td class="td_calendario" align="right">R$ <?php echo number_format($item['valor_liquido'],2,',','.'); ?></td>
                    <td class="td_calendario" align="left"><?php echo $item['juros']*100; ?>%</td>
               		<td class="td_calendario" align="right">R$ <?php echo number_format($item['valor_a_restituir'],2,',','.'); ?></td>
                    <td class="td_calendario" align="left"><?php echo $data->desconverterData($aux_data_devolucao[0]); ?></td>
                    <td class="td_calendario" align="right">R$ <?php echo number_format($item['IRRF_a_ser_pago'],2,',','.'); ?></td>
                    <td class="td_calendario" align="center">
                    	<?php if( floatval(str_replace('.','',$item['IOF_a_ser_recolhido'])) > 0 ){ ?>
                    ...
                    <td class="td_calendario" align="center">
                    	<?php if( floatval(str_replace('.', '', $item['IRRF_a_ser_pago'])) > 0 ){ ?>
