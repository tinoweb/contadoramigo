...
{        
    ...
    public function listarCategorias($tipo, $ano, $categoriaEmprestimo)
    {
		...
		// Eliminar esta query assim que for feita a correção das categirias. E devera descomentar a decima.
		$this->result = mysql_query(
			"SELECT categoria
			FROM user_" . $this->id . "_livro_caixa
			WHERE (" . $tipo . " > 0) AND (YEAR(data) = " . $ano . ") AND (Categoria <> '" . $categoriaEmprestimo . "' AND Categoria <> 'Pgto. de Salários' AND Categoria <> 'Licença de Software' AND Categoria <> 'Licença ou aluguel de softwares' AND Categoria <> 'Despesas bancárias' AND Categoria <> 'Juros' )
			GROUP BY categoria"
		)
