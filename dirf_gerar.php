...
//Verifica se as informações do banco não estão vazias
...
/* CAMPO REMOVIDO COM RELACÃO AO PADRÃO DA DIRF 2017 campo 11 */ //$identificador_pagamentos_copa = "N"; // S - Existe pagamento relacionado a copa / N - Não existe pagamento relacionado a copa

/* campo 11 */$identificador_pagamentos_olimpiadas = "N"; // S - Existe pagamento relacionado às Olimpíadas. N - Não existe pagamento relacionado às Olimpíadas. relacionado a copa

/* campo 12 */$identificador_capital_social_sujeito_voto = "N";

/* campo 13 */$identificador_situacao_declaracao = "N"; // S - Declaração de situação especial / N - Não é declaração de situação especial
/* campo 14 */$data_evento = ""; // OBRIGATORIO SE O CAMPO 12 = "S"
...
$decpj .= str_pad($identificador_capital_social_sujeito_voto,1," ",STR_PAD_RIGHT).$fim_campo;
$decpj .= str_pad($identificador_pagamentos_olimpiadas,1," ",STR_PAD_RIGHT).$fim_campo;
...
// AND (pgto.id_autonomo > 0 OR pgto.id_socio > 0 OR pgto.id_pj > 0 OR pgto.id_lucro > 0) // Condicão removida da query.
//AND (pgto.id_autonomo > 0 OR pgto.id_socio > 0 OR (pgto.id_pj > 0 AND pgto.IR > 0 AND  pgto.descricao_servico <> 'Plano ou Seguro Saúde Privado') OR pgto.id_lucro > 0)
//echo $sqlPAGTOS;
//exit;
...
while($rsPAGTOS = mysql_fetch_array($PAGTOS)){			
	...
	if($bdec_anterior != $rsPAGTOS['documento']){
		...
		if($rsPAGTOS['tipo'] != 'pj'){
			...
			/* campo 4 */$data_laudo = "";
			/* campo 5 */$Indicador_identificacao_alimentando = "N";
			/* campo 6 */$Indicador_identificacao_previdencia_complementar = "N";
			...
			$bpfdec .= str_pad($Indicador_identificacao_alimentando,1," ",STR_PAD_RIGHT).$fim_campo;
			$bpfdec .= str_pad($Indicador_identificacao_previdencia_complementar,1," ",STR_PAD_RIGHT).$fim_campo;
			$bpfdec .= $fim_linha;
...
while($rsPAGTOS = mysql_fetch_array($pagtoFuncionario)){
	...
	if(!$codigoReceita){
		...
		/* campo 1 */$identificador_registro = "IDREC";
		/* campo 2 */$codigo_receita = $codigoReceita; // DE ACORDO COM TABELA DE CODIGOS DE RECEITAS CONSTANTES NA IN QUE DISPOE SOBRE A DIRF
	...
	if(
		...
		|| $rsPAGTOS["VALOR_BRUTO_NOVEMBRO"] > 0
		|| $rsPAGTOS["VALOR_BRUTO_DEZEMBRO"] > 0
		|| $rsPAGTOS["VALOR_BRUTO_DECIMO_TERCEIRO"] > 0
	){
		/* RVM */
		...
		/* campo 1 */$identificador_registro = "RTRT"; // rtrt, rtpo, rtpp, rtdp, rtpa, rtirf, cjaa, cjac, esrt, espo, espp, esdp, esir, esdj, rip65, ridac, riirc, riap, rimog, rivc, ribmr, dajud
		$VALOR_BRUTO_JANEIRO = number_format($rsPAGTOS["VALOR_BRUTO_JANEIRO"], 2, '.', '');
		/* campo 2 */$janeiro = preg_replace("#\W#","",$VALOR_BRUTO_JANEIRO);
		
		$VALOR_BRUTO_FEVEREIRO = number_format($rsPAGTOS["VALOR_BRUTO_FEVEREIRO"], 2, '.', '');
		/* campo 3 */$fevereiro = preg_replace("#\W#","",$VALOR_BRUTO_FEVEREIRO);
				
		$VALOR_BRUTO_MARCO = number_format($rsPAGTOS["VALOR_BRUTO_MARCO"], 2, '.', '');
		/* campo 4 */$marco = preg_replace("#\W#","",$VALOR_BRUTO_MARCO);
		
		$VALOR_BRUTO_ABRIL = number_format($rsPAGTOS["VALOR_BRUTO_ABRIL"], 2, '.', '');
		/* campo 5 */$abril = preg_replace("#\W#","",$VALOR_BRUTO_ABRIL);
		
		$VALOR_BRUTO_MAIO = number_format($rsPAGTOS["VALOR_BRUTO_MAIO"], 2, '.', '');
		/* campo 6 */$maio = preg_replace("#\W#","",$VALOR_BRUTO_MAIO);
		
		$VALOR_BRUTO_JUNHO = number_format($rsPAGTOS["VALOR_BRUTO_JUNHO"], 2, '.', '');
		/* campo 7 */$junho = preg_replace("#\W#","",$VALOR_BRUTO_JUNHO);
		
		$VALOR_BRUTO_JULHO = number_format($rsPAGTOS["VALOR_BRUTO_JULHO"], 2, '.', '');
		/* campo 8 */$julho = preg_replace("#\W#","",$VALOR_BRUTO_JULHO);
		
		$VALOR_BRUTO_AGOSTO = number_format($rsPAGTOS["VALOR_BRUTO_AGOSTO"], 2, '.', '');
		/* campo 9 */$agosto = preg_replace("#\W#","",$VALOR_BRUTO_AGOSTO);
		
		$VALOR_BRUTO_SETEMBRO = number_format($rsPAGTOS["VALOR_BRUTO_SETEMBRO"], 2, '.', '');
		/* campo 10 */$setembro = preg_replace("#\W#","",$VALOR_BRUTO_SETEMBRO);
		
		$VALOR_BRUTO_OUTUBRO = number_format($rsPAGTOS["VALOR_BRUTO_OUTUBRO"], 2, '.', '');
		/* campo 11 */$outubro = preg_replace("#\W#","",$VALOR_BRUTO_OUTUBRO);
		
		$VALOR_BRUTO_NOVEMBRO = number_format($rsPAGTOS["VALOR_BRUTO_NOVEMBRO"], 2, '.', '');
		/* campo 12 */$novembro = preg_replace("#\W#","",$VALOR_BRUTO_NOVEMBRO);
		
		$VALOR_BRUTO_DEZEMBRO = number_format($rsPAGTOS["VALOR_BRUTO_DEZEMBRO"], 2, '.', '');
		/* campo 13 */$dezembro = preg_replace("#\W#","",$VALOR_BRUTO_DEZEMBRO);
		
		$VALOR_BRUTO_DECIMO_TERCEIRO = number_format($rsPAGTOS["VALOR_BRUTO_DECIMO_TERCEIRO"], 2, '.', '');
		/* campo 14 */$decimo_terceiro = preg_replace("#\W#","",$VALOR_BRUTO_DECIMO_TERCEIRO);
	...
	if(
		...
		|| $rsPAGTOS["INSS_NOVEMBRO"] > 0
		|| $rsPAGTOS["INSS_DEZEMBRO"] > 0
		|| $rsPAGTOS["INSS_DECIMO_TERCEIRO"] > 0
	){
		/* RVM */
		...
		$INSS_JANEIRO = number_format($rsPAGTOS["INSS_JANEIRO"], 2, '.', '');
		/* campo 2 */$janeiro = preg_replace("#\W#","",$INSS_JANEIRO);
		
		$INSS_FEVEREIRO = number_format($rsPAGTOS["INSS_FEVEREIRO"], 2, '.', '');
		/* campo 3 */$fevereiro = preg_replace("#\W#","",$INSS_FEVEREIRO);
		
		$INSS_MARCO = number_format($rsPAGTOS["INSS_MARCO"], 2, '.', '');
		/* campo 4 */$marco = preg_replace("#\W#","",$INSS_MARCO);
		
		$INSS_ABRIL = number_format($rsPAGTOS["INSS_ABRIL"], 2, '.', '');
		/* campo 5 */$abril = preg_replace("#\W#","",$INSS_ABRIL);
		
		$INSS_MAIO = number_format($rsPAGTOS["INSS_MAIO"], 2, '.', '');
		/* campo 6 */$maio = preg_replace("#\W#","",$INSS_MAIO);
		
		$INSS_JUNHO = number_format($rsPAGTOS["INSS_JUNHO"], 2, '.', '');
		/* campo 7 */$junho = preg_replace("#\W#","",$INSS_JUNHO);
		
		$INSS_JULHO = number_format($rsPAGTOS["INSS_JULHO"], 2, '.', '');
		/* campo 8 */$julho = preg_replace("#\W#","",$INSS_JULHO);
		
		$INSS_AGOSTO = number_format($rsPAGTOS["INSS_AGOSTO"], 2, '.', '');
		/* campo 9 */$agosto = preg_replace("#\W#","",$INSS_AGOSTO);
		
		$INSS_SETEMBRO = number_format($rsPAGTOS["INSS_SETEMBRO"], 2, '.', '');
		/* campo 10 */$setembro = preg_replace("#\W#","",$INSS_SETEMBRO);
		
		$INSS_OUTUBRO = number_format($rsPAGTOS["INSS_OUTUBRO"], 2, '.', '');
		/* campo 11 */$outubro = preg_replace("#\W#","",$INSS_OUTUBRO);
		
		$INSS_NOVEMBRO = number_format($rsPAGTOS["INSS_NOVEMBRO"], 2, '.', '');
		/* campo 12 */$novembro = preg_replace("#\W#","",$INSS_NOVEMBRO);
		
		$INSS_DEZEMBRO = number_format($rsPAGTOS["INSS_DEZEMBRO"], 2, '.', '');
		/* campo 13 */$dezembro = preg_replace("#\W#","",$INSS_DEZEMBRO);
		
		$INSS_DECIMO_TERCEIRO = number_format($rsPAGTOS["INSS_DECIMO_TERCEIRO"], 2, '.', '');
		/* campo 14 */$decimo_terceiro = preg_replace("#\W#","",$INSS_DECIMO_TERCEIRO);
	...
	if(
		...
		$desconto_dependentes_JANEIRO = number_format($rsPAGTOS["desconto_dependentes_JANEIRO"], 2, '.', '');
		/* campo 2 */$janeiro = preg_replace("#\W#","",$desconto_dependentes_JANEIRO);
		
		$desconto_dependentes_FEVEREIRO = number_format($rsPAGTOS["desconto_dependentes_FEVEREIRO"], 2, '.', '');
		/* campo 3 */$fevereiro = preg_replace("#\W#","",$desconto_dependentes_FEVEREIRO);
		
		$desconto_dependentes_MARCO = number_format($rsPAGTOS["desconto_dependentes_MARCO"], 2, '.', '');
		/* campo 4 */$marco = preg_replace("#\W#","",$desconto_dependentes_MARCO);
		
		$desconto_dependentes_ABRIL = number_format($rsPAGTOS["desconto_dependentes_ABRIL"], 2, '.', '');
		/* campo 5 */$abril = preg_replace("#\W#","",$desconto_dependentes_ABRIL);
		
		$desconto_dependentes_MAIO = number_format($rsPAGTOS["desconto_dependentes_MAIO"], 2, '.', '');
		/* campo 6 */$maio = preg_replace("#\W#","",$desconto_dependentes_MAIO);
		
		$desconto_dependentes_JUNHO = number_format($rsPAGTOS["desconto_dependentes_JUNHO"], 2, '.', '');
		/* campo 7 */$junho = preg_replace("#\W#","",$desconto_dependentes_JUNHO);
		
		$desconto_dependentes_JULHO = number_format($rsPAGTOS["desconto_dependentes_JULHO"], 2, '.', '');
		/* campo 8 */$julho = preg_replace("#\W#","",$desconto_dependentes_JULHO);
		
		$desconto_dependentes_AGOSTO = number_format($rsPAGTOS["desconto_dependentes_AGOSTO"], 2, '.', '');
		/* campo 9 */$agosto = preg_replace("#\W#","",$desconto_dependentes_AGOSTO);
		
		$desconto_dependentes_SETEMBRO = number_format($rsPAGTOS["desconto_dependentes_SETEMBRO"], 2, '.', '');
		/* campo 10 */$setembro = preg_replace("#\W#","",$desconto_dependentes_SETEMBRO);
		
		$desconto_dependentes_OUTUBRO = number_format($rsPAGTOS["desconto_dependentes_OUTUBRO"], 2, '.', '');
		/* campo 11 */$outubro = preg_replace("#\W#","",$desconto_dependentes_OUTUBRO);
		
		$desconto_dependentes_NOVEMBRO = number_format($rsPAGTOS["desconto_dependentes_NOVEMBRO"], 2, '.', '');
		/* campo 12 */$novembro = preg_replace("#\W#","",$desconto_dependentes_NOVEMBRO);
		
		$desconto_dependentes_DEZEMBRO = number_format($rsPAGTOS["desconto_dependentes_DEZEMBRO"], 2, '.', '');
		/* campo 13 */$desconto_dependentes_DEZEMBRO = preg_replace("#\W#","",$desconto_dependentes_DEZEMBRO);
		
		$desconto_dependentes_DECIMO_TERCEIRO = number_format($rsPAGTOS["desconto_dependentes_DECIMO_TERCEIRO"], 2, '.', '');
		/* campo 14 */$decimo_terceiro = preg_replace("#\W#","",$desconto_dependentes_DECIMO_TERCEIRO);
	...
	if(
		...
		$IR_JANEIRO = number_format($rsPAGTOS["IR_JANEIRO"], 2, '.', '');
		/* campo 2 */$janeiro = preg_replace("#\W#","",$IR_JANEIRO);
		
		$IR_FEVEREIRO = number_format($rsPAGTOS["IR_FEVEREIRO"], 2, '.', '');
		/* campo 3 */$fevereiro = preg_replace("#\W#","",$IR_FEVEREIRO);

		$IR_MARCO = number_format($rsPAGTOS["IR_MARCO"], 2, '.', '');
		/* campo 4 */$marco = preg_replace("#\W#","",$IR_MARCO);
		
		$IR_ABRIL = number_format($rsPAGTOS["IR_ABRIL"], 2, '.', '');
		/* campo 5 */$abril = preg_replace("#\W#","",$IR_ABRIL);
		
		$IR_MAIO = number_format($rsPAGTOS["IR_MAIO"], 2, '.', '');
		/* campo 6 */$maio = preg_replace("#\W#","",$IR_MAIO);
		
		$IR_JUNHO = number_format($rsPAGTOS["IR_JUNHO"], 2, '.', '');
		/* campo 7 */$junho = preg_replace("#\W#","",$IR_JUNHO);
		
		$IR_JULHO = number_format($rsPAGTOS["IR_JULHO"], 2, '.', '');
		/* campo 8 */$julho = preg_replace("#\W#","",$IR_JULHO);
		
		$IR_AGOSTO = number_format($rsPAGTOS["IR_AGOSTO"], 2, '.', '');
		/* campo 9 */$agosto = preg_replace("#\W#","",$IR_AGOSTO);
		
		$IR_SETEMBRO = number_format($rsPAGTOS["IR_SETEMBRO"], 2, '.', '');
		/* campo 10 */$setembro = preg_replace("#\W#","",$IR_SETEMBRO);
		
		$IR_OUTUBRO = number_format($rsPAGTOS["IR_OUTUBRO"], 2, '.', '');
		/* campo 11 */$outubro = preg_replace("#\W#","",$IR_OUTUBRO);
		
		$IR_NOVEMBRO = number_format($rsPAGTOS["IR_NOVEMBRO"], 2, '.', '');
		/* campo 12 */$novembro = preg_replace("#\W#","",$IR_NOVEMBRO);
		
		$IR_DEZEMBRO = number_format($rsPAGTOS["IR_DEZEMBRO"], 2, '.', '');
		/* campo 13 */$dezembro = preg_replace("#\W#","",$IR_DEZEMBRO);
		
		$IR_DECIMO_TERCEIRO = number_format($rsPAGTOS["IR_DECIMO_TERCEIRO"], 2, '.', '');
		/* campo 14 */$decimo_terceiro = preg_replace("#\W#","",$IR_DECIMO_TERCEIRO);
...
/**** Fim inclusão do pagamento do funcionario. ***************************************************/ 
...
/* campo 7 */$Valor_pago_para_advogado = "0";
/*
