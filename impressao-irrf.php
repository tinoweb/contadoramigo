...
//	$data_apuracao = $data->desconverterData(calcularDataApuracao($objeto['data'])); // A data de apuração e utiliza a data de pagamento.
	$data_apuracao = $data->desconverterData(calcularDataApuracao($objeto['data_pagamento']));
	$cpf_cnpj = '';
	if( $objeto['tipo'] == 1 )
		$codigo_receita = '8053';	
	else
		$codigo_receita = '3426';	
	$numero_referencia = '';
	$data_vencimento = calcularDataVencimento($objeto['data_pagamento']);
	$valor_principal = number_format($objeto['valor'],2,',','.');
	$valor_principal = number_format($objeto['IRRF_a_ser_pago'],2,',','.');//calcularIRRF($objeto);
	$valor_multa = '';
	$valor_juros = '';
