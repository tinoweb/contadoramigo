...
//		alert(URL);
	...
	function formSubmit(){   
		...
		if( validElement('txtRazaoSocial', msg1) == false){erro = true; return false}
		//if( validElement('txtNomeFantasia', msg1) == false){erro = true; return false}
		if( validElement('txtCNPJ', msg1) == false){erro = true; return false}
		if($('#txtCNPJ').val().length != 18){
			alert('Digite o CNPJ corretamente.'); 
			erro = true;
			return false;
		}
		if( validElement('txtCNAE_Principal', msg1) == false){erro = true; return false}
		...
		if( validElement('txtEndereco', msg1) == false){erro = true; return false}
		if( validElement('txtBairro', msg1) == false){erro = true; return false}
		if( validElement('txtCEP', msg1) == false){erro = true; return false}

		if( validElement('txtCidade', msg1) == false){erro = true; return false}
...
//		});
			...
			$.ajax({
				...
				success: function(retorno){
					if(retorno.status){
						...
						<? } else { ?>
							$('#aviso-empresa-alterada').fadeIn(100);
						<? } ?>
						$("#btSalvar").parent().find('.divCarregando2').css('display', 'none');
		...
		$('#btCadastrarNovaEmpresa').bind('click',function(){
			...
			location.href="lucas_meus_dados_empresa.php?act=new";
	...
	$('#btCancelar').click(function(){
		location.href = 'lucas_meus_dados_empresa.php';
	});
...
<div style="width:310px; position:absolute; display:none;" id="nire" class="bubble_left box_visualizacao x_visualizacao">
	...
	<div style="padding:20px;">
        ...
        <br />
        Você pode encontrá-lo em seu contrato social ou, se for firma individual, em sua certidão de registro na Junta.    
    </div>
...
<?=$textoAcao?></div>
  ...
  <input type="hidden" name="acao" value="<?=$acao?>" />
   ...
   <tr>
     ...
     <td class="formTabela"><input name="txtRazaoSocial" id="txtRazaoSocial" type="text" style="width:300px; margin-bottom:0px" value="<?=$razao_social?>" maxlength="125"  alt="Razão Social" /></td>
   </tr>
    <tr>
      <td align="right" valign="middle" class="formTabela">Nome Fantasia:</td>
      <td class="formTabela"><input name="txtNomeFantasia" id="txtNomeFantasia" type="text" style="width:300px" value="<?=$nome_fantasia?>" maxlength="200" alt="Nome Fantasia"  /></td>
    </tr>
    <tr>
  ...
  ?>
    <tr>
      <td align="right" valign="top" class="formTabela">* Atividade Principal (CNAE):</td><td  class="formTabela"> <input name="txtCNAE_Principal" id="txtCNAE_Principal" type="text" style="width:84px; float:left; margin-right:3px" value="<?=$linha2["cnae"];?>" class="campoCNAE" div="atividadePrincipal" check="hddCNAEPrincipal" alt="Atividade Principal (CNAE)">
      <input type="hidden" name="hddCNAEPrincipal" id="hddCNAEPrincipal" value="<?=$linha2["cnae"];?>">
        ...
        <div id="atividadePrincipal" style="float:left; margin-top:5px; margin-bottom:-5px"><?=$linha2['denominacao']?></div>
      </td>
      <?php	
  ...
  <?php 
    ...
    <tr>
      <td align="right" valign="top" class="formTabela">* Atividade Principal (CNAE):</td><td  class="formTabela"> <input name="txtCNAE_Principal" id="txtCNAE_Principal" type="text" style="width:84px; float:left; margin-right:3px" value="" class="campoCNAE" div="atividadePrincipal" check="hddCNAEPrincipal" alt="Atividade Principal (CNAE)">
  ...
  <!--o valor do campo count depende da variavel totalresultados, que é o numero de linhas cnae no banco de dados. Se for zero, deve ser um, de forma a gerar 1 campo de input para ser preenchido pelo usuario-->
    ...
    <tr>
      ...
      <td class="formTabela"><input name="txtInscricaoCCM" id="txtInscricaoCCM" type="text" style="width:95px; float:left; margin-right:3px" value="<?=$inscricao_no_ccm?>" maxlength="11" class="inteiro" />
        <div style="float:left; margin-right:5px; margin-top:5px; display: none;"><a href="javascript:abreDiv('ccm');reposicionaBallons()"><img src="images/dica.gif" width="13" height="14" border="0" align="texttop" id="dicaCCM" /></a></div></td>
    </tr>
    <tr>
  ...
  ?>
    ...
    <tr>
      ...
      <td class="formTabela"><!--<input name="txtCidade" id="txtCidade" type="text" style="width:300px" value="<?=$cidade?>" maxlength="200" alt="Cidade" />-->
    ...
    <tr>
      ...
      <td valign="middle" class="formTabela">
        <div style="float:left; margin-right: 3px;"><input name="txtPrefixoTelefone" id="txtPrefixoTelefone" type="text" style="width:30px" value="<?=$pref_telefone?>" maxlength="2" class="inteiro" /></div>
        <div style="float:left"><input name="txtTelefone" id="txtTelefone" type="text" style="width:75px" value="<?=$telefone?>" maxlength="9" class="inteiro" /></div></td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td class="formTabela"><select name="selInscritaComo" id="selInscritaComo" onchange="selecionaRegistro()" style="float:left">
        <option value="Empresa Individual" <?php echo selected( 'Empresa Individual', $inscrita_como ); ?>  onchange="selecionaRegistro()" >Empresário Individual</option>
        <option value="EIRELI" <?php echo selected( 'EIRELI', $inscrita_como ); ?>  onchange="selecionaRegistro()" >EIRELI</option>
        <option value="Sociedade Empresária Limitada" <?php echo selected( 'Sociedade Empresária Limitada', $inscrita_como ); ?> onchange="selecionaRegistro()">Sociedade Empresária Limitada (registrada na Junta Comercial)</option>
        <option value="Sociedade Simples" <?php echo selected( 'Sociedade Simples', $inscrita_como ); ?> onchange="selecionaRegistro()" >Sociedade Simples (registrada em cartório)</option>
      </select>
        <div style="float:left; margin-left:5px; margin-top:5px">
          <img class="imagemDica" src="images/dica.gif" width="13" height="14" border="0" align="texttop" div="tipo_emp" />
        </div>
      </td>
    </tr>
    <tr>
      <td colspan="2" class="formTabela">
            <table id="divRegistroNire" border="0" cellpadding="0" cellspacing="3" width="100%" style="background:none<?php if ($inscrita_como == 'Sociedade Simples' ) { echo '; display:none'; } ?>" class="formTabela">
      <tr>
        ...
        <table id="divRegistroCartorio" border="0" cellpadding="0" cellspacing="3" width="100%" style="background:none<?php if ($inscrita_como != 'Sociedade Simples' ) { echo '; display:none'; } ?>" class="formTabela">
      <tr>
          <td align="right" valign="middle" class="formTabela" width="196px">Número do Cartório:</td>
      ...
      <tr>
        <td align="right" valign="middle" class="formTabela">Número de registro no Cartório:</td>
        <td class="formTabela"><input name="txtRegistroCartorio" id="txtRegistroCartorio" type="text" style="width:150px; float:left; margin-right:3px" value="<?=$registro_cartorio?>" maxlength="20" alt="Número de registro" /></td>
        </tr>
        </table>
      </td>
      </tr>
...
<!--    <tr>
	  ...
	  <td class="formTabela">
	  	<input type="radio" name="txt_recolhe_cprb" value="1" />Sim&nbsp;&nbsp; 
	  	<input type="radio" name="txt_recolhe_cprb" value="0" />Não
	  </td>-->
    </tr>
    <tr>
  ...
  <tr>
    ...
    <td class="formTabela">
			...
			<div class="divCarregando2" style="margin-top: 10px;text-align: center;display: none;float: left;margin-right: 20px;margin-top: 5px;"><img src="images/loading.gif" width="16" height="16"></div>
...
<br />
  ...
  ?>
  	<div style="text-align: left; margin-bottom:10px; width:75%"><a href="lucas_meus_dados_empresa.php?act=new">Cadastrar nova Empresa</a></div>
	<?	
  }
...
<div style="text-align: right; margin-bottom:10px; width:99.8%"><a href="lucas_meus_dados_empresa.php?act=new">Cadastrar <?=$_SESSION['n_empresasVinculadas'] > 0 ? 'outra' : ''?> empresa</a></div>
...
?>
            <tr>
                <td class="td_calendario" align="center">
                  <? if($ativa == 1){ ?>
                  	<a href="lucas_meus_dados_empresa.php?editar=<?=$id?>">
                ...
                <td class="td_calendario"><?=$nome_fantasia?></td>
