<?php 
	...
	class EMKT{
		...
		function criarMensagem($mensagem){
			...
			$string = '{ "message": { "domain_id": "106156", "html_body": "'.$mensagem->gethtml_body().'", "text_body": "'.$mensagem->gettext_body().'", "list_ids": ["'.$mensagem->getlist_ids().'"], "name": "'.$mensagem->getname().'", "sender_name": "'.$mensagem->getsender_name().'", "sender": "'.$mensagem->getsender().'", "subject": "'.$mensagem->getsubject().'", "scheduled_to": "'.$mensagem->getscheduled_to().'" } } ';
			return $string;
	...
	class Lista{
		...
		function iflistasParaRemoverDiaria($string){
			...
			if( $string == 'user_vencidos' )
				return true;
			if( $string == 'premium_contratado' )
				return true;
			if( $string == 'servico_contratado' )
				return true;			
...
//			echo "<pre> sendObject: CURLOPT_URL - ";
//				print_r('$resultado_http: '.$resultado_http);
//			echo "</pre>";
...
//			echo "<pre> Método: agendarMensagem - ";
//				print_r('$tipo_mensagem: '.$tipo_mensagem.' $id_lista: '.$id_lista);
//			echo "</pre>";
			...
			$mensagem->setscheduled_to(date("Y-m-d H:i:s",strtotime("+15 minutes")));
			$json->setobject($emkt->criarMensagem($mensagem));
			$json->settipo("newMensagem");
			
			//echo "Linha 475 agendarMensagem - sendObject";
...
//			echo "<pre> Retorno da Lista: ";
//				print_r('$id_lista: '.$id_lista);
//			echo "</pre>";
