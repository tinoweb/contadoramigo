...
<div class="principal minHeight">
  ...
  <h2>Envio da Gfip</h2>
	...
	<div class="quadro_branco"><strong>ATENÇÃO</strong>: antes de enviar a sua próxima Gfip, atualize o programa Sefip. Ele foi alterado para atender as novas exigências da reforma trabalhista. Para a instalação da nova versão será necessário primeiro desinstalar a atual e rodar o script de limpeza. Proceda da seguinte forma:
	  <ol>
			<li>Desinstale o Sefip de sua máquina</li>
			<li>Baixe, descompacte e execute o <a href="http://www.caixa.gov.br/Downloads/fgts-sefip-grf/ScriptSEFIP.zip">Script de Limpeza</a> (o arquivo tem a extensão .bat é muito rápido quase não dá para perceber que rodou)</li>
			<li>Baixe e instale a <a href="http://www.caixa.gov.br/Downloads/fgts-sefip-grf/SetupSefipV84.exe">Nova Versão do Sefip</a></li>
		</ol>
	Quando for enviar sua próxima Gfip, no início do tutorial do Contador Amigo, responda SIM à pergunta: Está enviando a Gfip pela primeira vez com o Contador Amigo, formatou seu micro ou está usando um novo computador? Dessa forma você será orientado como proceder. Em caso de dúvida, consulte nosso <a href="suporte.php">Help Desk</a>. </div>
...
$(document).ready(function(e) {
			...
			function acaoButton() {
				...
				// Bloqueio temporario.
				$('.obraRegistradaSim').each(function(){
				...
				if(status) {
					alert('Para enviar esta gfip solicite auxílio do Help Desk.');
					return false;
				}
...
//				});
	...
	function escolhe_destino() {
			...
			if( p_vez == 'nao' && movimento == 'sim' ){
				$.ajax({
					url: 'SEFIP_config.php',
					data: {pgDestino:'gfip_orientacoes_envio_normal.php', p_vez:'nao'},
					dataType:"text",
					type:"POST",
					cache: false,
					success: function(response){
			...
			if(p_vez == 'sim' && movimento == 'sim' ){
				$.ajax({
					url: 'SEFIP_config.php',
					data: {pgDestino:'gfip_orientacoes_primeira_vez_normal.php',p_vez:'sim'},
					dataType:"text",
					type:"POST",
					cache: false,
					success: function(response){
...
//			}
    ...
    <div id="aviso_ok">
        ...
        <div style="margin-bottom:20px">
          <p>A Gfip é uma declaração na qual você informa à Receita Federal quanto retirou de pró-labore e os salários pagos no período. Com base nessa informação, será calculado o recolhimento do INSS, que corresponde a 11% do valor declarado. Assim,<strong> antes  de iniciar o processo de geração e envio da Gfip, você precisa primeiro cadastrar o(s) pró-labore(s) dos sócios e/ou proprietário, salários e remunerações a autônomos no período</strong>. Faça isso na <strong>aba Pagamentos</strong> do menu acima. Caso não tenha havido nenhum desses pagamentos no período, será enviada uma Gfip sem movimento. Vale lembrar o período sem movimento não será contado como tempo de contribuição para sua aposentadoria.
          <br><br>
  ...
  <br>
<div class="destaqueAzul" style="margin-bottom: 10px">Gfip 13</div>
     Refere-se ao 13º salário e é declarada em janeiro (portanto em janeiro serão enviadas 2 Gfips). Como sócios e autônomos não têm 13º, se a sua empresa não tiver funcionários, deve ser enviada uma Gfip sem movimento.
	    ...
	    <div class="quadro_passos_sem_largura" style="padding:40px;">
	    	...
	    	<div style="display: block;">
                ...
                <div  style="margin-bottom:20px">
                    <div class="perguntas_principais_quadros" style="float:left; width:600px; min-width: 300px">Está enviando a Gfip pela primeira vez com o Contador Amigo,  formatou seu micro ou está usando um novo computador?</div>
...
<? if(mysql_num_rows($checa_cnaes_anexoIV) > 0) { ?>
    ...
    ?>	
                ...
                <div style="margin:10px 0px 10px 30px; display: none;" class="opcao_simples" id="<?=$linha2["cnae_limpo"]?>">
                    ...
                    <input class="abrir_cprb_input obraRegistradaSim" name="ativ_<?=$linha2["cnae_limpo"]?>" type="radio" value="IV" /> Sim (selecione esta opção se estiver executando o serviço para uma obra registrada)
                    <div style="clear: both; height: 5px" ></div>
