...
// PRECISA SER CRIADA UMA CONEXAO NESTE PONTO POIS REMOS UMA CONSULTA A FAZER
...
$criterios_transporte = array('4911600','4911600','4912401','4921302','4922101','4922102','4922103','4929902','4929904','4930202','5012201','5012202','5021102','5022002','5091202','5099801','5111100','5112999','5120000','5130700');
...
$criterios_locacao = array('7711000','7719501','7719502','7719599','7721700','7722500','7723300','7729201','7729202','7729203','7729299','7731400','7732201','7733100','7739001','7739002','7739099');
...
$criterios_dubios = array('4912403','4929999','4930203','4930204','4940000','5011401','5011402','5030101','5030102','5099899');
...
foreach($_SESSION['cnaes_empresa_mes'] as $cnae){
	...
	//if(isset($_POST['ativ_'.$CNAE])){
		// INSERINDO DADOS NA TABELA COM AS ATIVIDADES DO MÊS - SERÁ UTILIZADO NA PÁGINA IMPOSTOS_ALIQUOTAS
		//mysql_query("INSERT INTO dados_atividades_periodo (id, cnae, anexo) VALUES (".$_SESSION["id_empresaSecao"].",'".$cnae."','".$_POST['ativ_'.$CNAE]."')");
	//}else{
		$anexo_cnae = mysql_fetch_array(mysql_query("SELECT * FROM cnae WHERE cnae = '" .  $cnae . "'"));	
		// INSERINDO DADOS NA TABELA COM AS ATIVIDADES DO MÊS - SERÁ UTILIZADO NA PÁGINA IMPOSTOS_ALIQUOTAS
		//mysql_query("INSERT INTO dados_atividades_periodo (id, cnae, anexo) VALUES (".$_SESSION["id_empresaSecao"].",'".$cnae."','".$anexo_cnae['anexo']."')");
	//}

	// CASO SEJA O CNAE DE CONTABILIDADE E TENHA SIDO SELECIONADO O RADIO "não", ESTE CNAE NÃO ENTRA EM NENHUM ARRAY DE CONFERÊNCIA
...
if(count($_SESSION['cnaes_empresa_mes']) == 1
	...
	$descr_cnae = mysql_fetch_array(mysql_query("SELECT * FROM cnae WHERE cnae = '" .  $_SESSION['cnaes_empresa_mes'][0] . "'"));
//	echo "SELECT * FROM cnae WHERE cnae = '" .  $_SESSION['cnaes_empresa_mes'][0] . "'";

	// SE FOI SELECIONADO SOMENTE UMA ATIVIDADE 
	//if($_POST['ativ_'.$CNAE] == "IIIc"){

	//	$_SESSION['passou_direto'] = 1;
		
	//	$_SESSION['anexo_'.$CNAE] = "III";
	//	$_SESSION['marcar_check_'.$CNAE] = "11;12";
	//  header('location: simples_orientacoes_normal.php');
		
	//}else if(in_array($CNAE,$criterios_locacao)){
		// AQUI DEVE SER FEITA UMA PERGUNTA
		
	//	$_SESSION['passou_direto'] = 1;

	//	$_SESSION['anexo_'.$CNAE] = $descr_cnae['anexo'];
	//	$_SESSION['marcar_check_'.$CNAE] = "9";
	//  header('location: simples_orientacoes_normal.php');

	//}
	/*else if($descr_cnae['anexo'] == 'VI'){
		$_SESSION['passou_direto'] = 1;

		$_SESSION['anexo_'.$CNAE] = $descr_cnae['anexo'];
		$_SESSION['marcar_check_'.$CNAE] = "";
	    header('location: simples_orientacoes_normal.php');
...
?>
	...
	<form id="form_anexos" action="simples_orientacoes_normal.php" method="post">
...
<?
	...
	//$CNAE = '4912403';
		...
		if(in_array($anexo_CNAE,$anexos3456)){
			...
			if(in_array($CNAE,$criterios_transporte)){
...
?>
        ...
        $(document).ready(function(e) {
					...
					campo_hidden<?=$CNAE?> = $('input[name=marcar_check_<?=$CNAE?>]');

					$('input:radio[name=opcoes_<?=$CNAE?>]').click(function(){

						opcoes = ($('input:radio[name=opcoes_<?=$CNAE?>]:checked').index('input[name=opcoes_<?=$CNAE?>]'));
						...
						//alert(campo_hidden<?=$CNAE?>.val());
						
					});
				});
        </script>
        <div style="margin:10px 0 20px 30px;" class="opcao_simples" >
          Algum cliente reteve o ISS ao pagar sua nota desta atividade?<br />
          <div style="clear: both; height: 5px" ></div>
          <input name="opcoes_<?=$CNAE?>" type="radio" /> <strong>NENHUMA</strong> nota teve retenção.
          <div style="clear: both; height: 5px" ></div>
          <input name="opcoes_<?=$CNAE?>" type="radio" /> <strong>ALGUMAS</strong> notas tiveram e outras não.
...
/*						if(campo_hidden<?=$CNAE?>.val() != ''){*/
						...
						$('input:radio[name=vendida_em_<?=$CNAE?>], input:radio[name=subst_tributaria_<?=$CNAE?>]').click(function(){
							...
							//alert('vendida_em: ' + vendida_em);
							//alert('subst_tributaria: ' + subst_tributaria);
