...
WHERE
	cob.resultado_acao in ('1.2','2.1','9.4','9.5','9.6','9.7')
	AND cob.emissao_NF = 0
...
if($dados_cobranca['tipo_plano'] == 'P'){
...
/* campo 8 */$valor_deducoes = "";
	...
	//Verifica o boleto e mensal ou avulço para definir o código de serviço prestado.
	if(in_array($dados_cobranca['resultado_acao'],$resultadoAcaoArr)){
		/* campo 9 */$codigo_servico_prestado = "2798";
	} else {
		/* campo 9 */$codigo_servico_prestado = "6298";
	}
...
/* campo 37 */$campo_reservado = "";
	...
	// Verifica o boleto e mensal ou avulço para definir a descrição e a Aliquota de inpostos.
	if(in_array($dados_cobranca['resultado_acao'],$resultadoAcaoArr)){
...
Alíquotas de impostos: IRPJ: 0,00% - CSLL: 0,00% - COFINS: 0,00% - PIS: 0,00% - CPP: 4,00% - ISS: 2,00%");
		...
		/* campo 38 */$descricao_servicos = preg_replace('/[\n]+?/','|',"Intermediação de serviços contábeis
...
//			alert(url + separador + "toggleRPS=" + id_pagto);
    ...
    $('#bt_gerar_arquivo').click(function(e){
		...
		//alert(url + separador + "processar"+ids);
