...
function ProcessaArquivoRetorno($vTemp_name, $vNome_arquivo, $ignora_upload, $vMensalidade, $vShow_log){	
	...
	for($i=0; $i<=($total_linhas_arquivo); $i++)
	{
		...
		if(substr($arquivo[$i],13,1) == "T"){
			...
			$planoAssinado = mysql_fetch_array(pegaPlanoBoletosRegistrados($nosso_numero));
...
$tPlano = (isset($_GET["tPlano"])? $_GET["tPlano"] : '');
...
<script>
	...
	//	assinante = document.getElementById('assinante').value;
		...
		var tPlano = document.getElementById('tPlano').value;
		
		window.location='cobranca.php?dataInicio='+anoInicio+'-'+mesInicio+'-'+diaInicio+'&dataFim='+anoFim+'-'+mesFim+'-'+diaFim+'&acao='+acao+'&tid='+tid+'&id='+id+'&email='+email+'&tipoData='+tipoData+'&tPlano='+tPlano;
	}
...
<div class="titulo" style="margin-bottom: 0px">Cobrança</div>

	<center><input type="button" value="Rodar Cobrança" id="btIniciarCobranca" style=""></center>
...
<div style="position: relative; font-size: 11px;margin-top:10px;">
    ...
    TID  
    <input name="tid" id="tid" type="text" value="<?=($tid)?>" maxlength="35" style="width:100px; font-size: 11px;" class="campoTID" />  
    E-mail 
    <input type="text" name="email" id="email" maxlength="220" style="font-size: 11px;width:100px;" />  
    ID  
    <input name="id" id="id" type="text" value="<?=($id)?>" maxlength="10" style="width:30px; font-size: 11px;" class="" /> 
    <!-- <div id="preenchimentoBusca" style="position: absolute;"></div> -->
      Tipo pgto.
    <select name="selAcao" id="selAcao" style="font-size: 11px; width: 80px;"> 
    <?
    /*
    ...
    */
    ...
    <option value="data" <?php echo selected( 'data', $tipoData ); ?>>Vendidos</option>
    <option value="data_pagamento" <?php echo selected( 'data_pagamento', $tipoData ); ?>>Pagos</option>
    </select>
    plano
    <select id="tPlano" name="tPlano" style="font-size: 11px;">
    	<option value="T">Todos</option>
    	<option value="P" <?php echo ($tPlano == 'P'? 'selected' : '');?>>Premium</option>
    	<option value="S" <?php echo ($tPlano == 'S'? 'selected' : '');?>>Standard</option>
    </select>
    de  
      <input name="DataInicio" id="DataInicio" type="text" value="<?php echo $data_inicio_filtro; ?>" maxlength="10"  style="width:60px;font-size: 11px;" class="campoData"/> 
      até 
      <input name="DataFim" id="DataFim" type="text" value="<?php echo $data_fim_filtro; ?>" maxlength="10"  style="width:60px;font-size: 11px;margin-right:10px;" class="campoData" />
...
// MÉTODO CRIADO PARA GRAVAR OS EMAIL REFERENTE A INFORMAÇ`O
...
//	//Pega os demos inscritos  a mais de 15 dias para enviar um email
...
//	 error_reporting(E_ALL);
	...
	*/
	...
	// 07/02/2018 - atualiza os historico dos pagamento com boletos e sem forma de pagamento apos a data de vencimento e para cartão no mesmo dia do vencimento.  
		$sqlUpdateVencidos = "
			UPDATE historico_cobranca h 
			JOIN login l ON h.id = l.id 
			JOIN dados_cobranca c ON c.id = l.id
			SET h.status_pagamento = (CASE 
										WHEN l.status = 'demo' THEN 'não pago' 
										ELSE 'vencido' 
									END) 
			WHERE status_pagamento IN ('a vencer','pendente') 
			AND (DATEDIFF(data_pagamento, DATE(now())) < 0 AND c.forma_pagameto IN ('boleto','') OR DATEDIFF(data_pagamento, DATE(now())) <= 0 AND c.forma_pagameto IN ('visa','mastercard','amex','diners','elo'))
...
//	$rs_checa_cancelamento = mysql_query($sql_checa_cancelamento);
	...
	// LOOP PARA CANCELAMENTO DE USUARIOS - PARA SEGUNDA - quanquer não pago há mais de 31 dias
	
	// CHECANDO NO HISTORICO DE COBRANCA SE HÁ REGISTRO NÃO PAGO HA MAIS DE 1 MESES PARA DETERMINAR SE DEVE CANCELAR O USUARIO
	$sql_checa_cancelamento = "
								SELECT 
									distinct h.id 
								FROM login l
								INNER JOIN 
									historico_cobranca h ON l.id = h.id
								WHERE 
									DATEDIFF(h.data_pagamento, DATE(NOW())) < -31 
									AND h.status_pagamento =  'não pago'
									AND l.status <> 'cancelado' 
							";
	$rs_checa_cancelamento = mysql_query($sql_checa_cancelamento);
	...
	// ############################## LOG ###############################
	if($show_log == true){
		echo "foram localizados " . mysql_num_rows($rs_checa_cancelamento) . " usuários com pagamentos não pagos há mais de 1 mês<BR><BR>";
			ob_flush();
	}
	// LOG DE ACESSOS
...
//		include '../mensagens/componente_envio_novo.php';
	...
	*/
	$sql = "SELECT 
				...
				, d.plano
				, d.contadorId
				, d.desconto_mesalidade
				, d.desconto_S_mensal
				, d.desconto_S_trimestral
				, d.desconto_S_semestral
				, d.desconto_S_anual
				, d.desconto_P_mensal
				, d.desconto_P_trimestral
				, d.desconto_P_semestral
				, d.desconto_P_anual
			FROM 
			   	login l
			INNER JOIN historico_cobranca h ON l.id = h.id
			INNER JOIN dados_cobranca d ON h.id = d.id 
			WHERE (l.status <> 'cancelado' AND l.status <> 'demoInativo')
				...
				if(false){
					...
					if($dados_cartao){
						...
						*/	
						...
						$plano_usuario = $linha['plano'];
						$tipo_plano_usuario = $linha['tipo_plano'];
						$descontoMesalidade = $linha['desconto_mesalidade'];
						$plano_meses = $Config->verMeses($plano_usuario); //RECUPERA MESES DO PLANO
						
						// Pega o volor do plano e define qual devera ser o desconto.
						if($descontoMesalidade == 1) {
							$plano_valor = $Config->verValor($plano_usuario, $tipo_plano_usuario, $descontoMesalidade); // RECUPERA O VALOR DO PLANO
						} // Aplica o desconto individual. 
						elseif($descontoMesalidade == 2) {
							
							// Pega o valor do plano
							$plano_valor = $Config->verValor($plano_usuario, $tipo_plano_usuario);
							
							// Verifica o tipo do plano para aplica o desconto.
							switch($plano_usuario){
								case 'mensalidade':
									if($tipo_plano_usuario == 'P') {
										$plano_valor = $plano_valor - $linha['desconto_P_mensal'];
									} else {
										$plano_valor = $plano_valor - $linha['desconto_S_mensal'];
									}
									break;
								case 'trimestral':  
									if($tipo_plano_usuario == 'P') {
										$plano_valor = $plano_valor - $linha['desconto_P_trimestral'];
									} else {
										$plano_valor = $plano_valor - $linha['desconto_S_trimestral'];
									}	
									break;
									
								case 'semestral':   
									if($tipo_plano_usuario == 'P') {
										$plano_valor = $plano_valor - $linha['desconto_P_semestral'];
									} else {
										$plano_valor = $plano_valor - $linha['desconto_S_semestral'];
									}
									break;
								case 'anual':
									if($tipo_plano_usuario == 'P') {
										$plano_valor = $plano_valor - $linha['desconto_P_anual'];
									} else {
										$plano_valor = $plano_valor - $linha['desconto_S_anual'];
									}
									break;
							}
						...
						// Linha com a condição para alterar o retorno do pagamento para "empresa teste e contador amigo". 	
						if ( $linha["idUser"] == 9 || $linha["idUser"] == 1581 ) {
							$retorno_codigo_autorizacao = 6;
						}

						$nomePlano = 'Standard';
						
						// Define o tipo de pagamento atraves do tipo do plano Premium ou Standard
						if($linha["tipo_plano"] == 'P'){
							$nomePlano = 'Premium';
						}
						...
						if (($retorno_codigo_erro == '') && ($retorno_codigo_autorizacao != '5')) { //Código 5 equivale a transação não autorizada
							...
							if(is_null($retorno_tid) || $retorno_tid == ""){
								...
								$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, tipo, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.4', 'não enviado', '', " . $valor_pago . ", '" . $numeroCartao."','".$linha["tipo_plano"]."','".$linha["plano"]."','".$nomePlano."', 'Registro linha: 1967')";
								$resultadoup = mysql_query($sqlup)
								or die (mysql_error());
								...
								// VERIFICA SE O PAGAMENTO E DE UMA CONTA PREMIUM E GRAVA O CÓDIGO DO CONTADOR E DO RELATORIO DE COBRANCA.  
								if($linha["tipo_plano"] == 'P') {
									...
									$valorTotal = $valor_pago  - number_format(($valor_pago / 2),2,'.','');
									
									// Aplica o juros de 20% (vinte porcento) para pegar o valor líquido.
									$valorLiquido = $valorTotal - ($valorTotal * $desconto);
									
									$qryUpdate = "INSERT INTO `cobranca_contador` (`idRelatorio`,`contadorId`,`valor_total`,`valor_liquido`,`desconto_porcentagem`) VALUE (".$idRelatorio.", ".$linha['contadorId'].", ".$valorTotal.", ".$valorLiquido.", ".$desconto.");";
								...
								// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
								$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, tipo, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.1', 'enviado', '".$retorno_tid."', " . $valor_pago . ", '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."','".$nomePlano."','Registro linha: 2021')";
								$resultadoup = mysql_query($sqlup)
								or die (mysql_error());
						...
						//       Caso retorne com erro.          Caso o cartão ocorra uma transação não autorizada.
							...
							// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
							$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, numero_cartao, tipo_plano, plano, tipo, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '" . $acao_relatorio . "', 'enviado', '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."','".$nomePlano."','Registro linha: 2101')";
							$resultadoup = mysql_query($sqlup)
							or die (mysql_error());
						...
						// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
						$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, tipo, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.5', 'não enviado', '', " . $valor_pago . ", '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."','".$nomePlano."','Registro linha: 2131')";
						$resultadoup = mysql_query($sqlup)
						or die (mysql_error());
...
//				include '../mensagens/componente_envio_novo.php';
				...
				if(!in_array($checa_desativacao['forma_pagameto'], array('mastercard','visa','elo','amex','diners'))){
...
*/
		...
		if($insere_relatorio == true){
			...
			// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
			$sqlup = "INSERT INTO relatorio_cobranca (id, data, resultado_acao, tipo_cobranca, envio_email, observacao) VALUES ('" . $checa_desativacao["id"] . "', '" . date('Y-m-d') . "', '" . $acao_relatorio . "', '" . $checa_desativacao['forma_pagameto'] . "', 'enviado', 'Registro linha: 2406')";
			$resultadoup = mysql_query($sqlup)
			or die (mysql_error());
...
*/
	...
	*/
			...
			if($insere_relatorio == true){
				...
				// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
				$sqlup = "INSERT INTO relatorio_cobranca (id, data, resultado_acao, tipo_cobranca, envio_email, observacao) VALUES ('" . $checa_ativacao["id"] . "', '" . date('Y-m-d') . "', '" . $acao_relatorio . "','" . $checa_desativacao['forma_pagameto'] . "', 'enviado', 'Registro linha: 2710')";
				$resultadoup = mysql_query($sqlup)
				or die (mysql_error());
	...
	function enviarEmailVencidos(){
		...
		$consulta_demo_info = mysql_query("SELECT *,DATEDIFF(data_pagamento, DATE(now())) from historico_cobranca WHERE DATEDIFF(data_pagamento, DATE(now())) < -3 AND `status_pagamento` = 'vencido' ");
		
		while ( $objeto_demo_info = mysql_fetch_array($consulta_demo_info) ) {
			
			$consulta_email_login = mysql_query("SELECT * FROM login WHERE idUsuarioPai = id AND idUsuarioPai = '".$objeto_demo_info['id']."' AND status != 'cancelado' LIMIT 1");
			$objeto_email_login = mysql_fetch_array($consulta_email_login);

			if(isset($objeto_email_login['email']) && !empty($objeto_email_login['email'])){
...
//	$titulo_vermelho = "Busca por Assinante: " . $get_assinante . "";
	...
	if( $id != "" ){
		...
		if($tid != "") {
			...
			if($tPlano == 'P') {
				$sql .= " AND tipo_plano = 'P' AND ( resultado_acao = '1.2' OR resultado_acao = '2.1' )";	
			}elseif($tPlano == 'S') {
				$sql .= " AND tipo_plano = 'S' AND ( resultado_acao = '1.2' OR resultado_acao = '2.1' )";	
			}			
...
<table border="0" cellspacing="2" cellpadding="4" style="font-size:11px;" width="100%">
    <tr>
     <th width="8%" align="center">Venda</th>
    <th width="8%" align="center">Pgto.</th>
    <th width="24%" align="center">Razão Social</th>
<!--    <th width="12%" align="center">CNPJ</th>-->
    <th width="15%" align="center">TID</th>
<!--    <th align="center">Nº cartão</th>-->
    ...
    <th width="5%" align="center">nº NF</th>
    </tr> 
<?php
while ($linha=mysql_fetch_array($resultado)) {
...
//		} else {
			...
			else if(($linha["resultado_acao"] == "7.1" || $linha["resultado_acao"] == "8.2" || $linha["resultado_acao"] == "8.3" || $linha["resultado_acao"] == "8.4" || $linha["resultado_acao"] == "8.5" || $linha["resultado_acao"] == "8.6" || $linha["resultado_acao"] == "8.7" || $linha["resultado_acao"] == "8.8" || $linha["resultado_acao"] == "8.9" || $linha["resultado_acao"] == "9.0" || $linha["resultado_acao"] == "9.4" || $linha["resultado_acao"] == "9.5" || $linha["resultado_acao"] == "9.6" || $linha["resultado_acao"] == "9.7" || $linha["resultado_acao"] == "9.8" || $linha["resultado_acao"] == "11.0" || $linha["resultado_acao"] == "11.1") && $linha['emissao_NF'] == 0){
				echo "fdfddd";
			}  
...
-->
       ...
       <td><?php
			...
			// Recupera o nome do contador.
			$queryContador = "SELECT nome FROM dados_contador_balanco D JOIN cobranca_contador C ON C.contadorId = D.id WHERE C.idRelatorio = '".$linha['idRelatorio']."'";
			$resultContador = mysql_query($queryContador)or die (mysql_error());
			$dadoContador = mysql_fetch_array($resultContador);
	
			// Variaável que recebe o nome da ação.
			$tipo = $linha["tipo"];
		
			// Array criado para normalizar o nome dos tipo.			
			$arrayTipo = array('A_empresa'=>'Abertura I'
						 ,'A_sociedade'=>'Abertura S'
						 ,'Simples_DAS'=>'Simples e DAS'
						 ,'decore'=>'DECORE'
						 ,'Gfip_GPS'=>'Gfip'
						 ,'F_empresa'=>'Baixa I'
						 ,'F_sociedade'=>'Baixa S'
			...
			if(isset($dadoContador['nome']) && !empty($dadoContador['nome'])) {
				
				$nomeContador = explode(' ',$dadoContador['nome']);
				
				echo '<a title="Contador Responsável: '.$dadoContador['nome'].'" style="cursor:help;">'.$tipo.' | '.$nomeContador[0].'</a>';
			} else {
				echo '<a>'.$tipo.'</a>';
			}
...
//			case '1.2': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.1': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.2': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.3': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.4': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.5': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			case '2.6': $resultado_acao = ($linha["tipo_plano"] == 'P' ? 'Premium' : 'Standard'); break;
//			
//			case '7.1': $resultado_acao = 'Complementar'; break;
//			case '8.2': $resultado_acao = 'Gfip'; break;
//			case '8.3': $resultado_acao = 'Simples'; break;
//			case '8.4': $resultado_acao = 'Defis'; break;
//			case '8.5': $resultado_acao = 'Rais'; break;
//			case '8.6': $resultado_acao = 'Dirf'; break;
//			case '8.7': $resultado_acao = 'Baixa Individual'; break;
//			case '8.8': $resultado_acao = 'Baixa Sociedade'; break;
//			case '8.9': $resultado_acao = 'MEI para ME'; break;
//			case '9.0': $resultado_acao = 'CPOM'; break;
//			case '9.3': $resultado_acao = 'Avulso'; break;
//			case '9.4': $resultado_acao = 'Abertura Individual'; break;
//			case '9.5': $resultado_acao = 'Abertura Sociedade'; break;
//			case '9.6': $resultado_acao = 'DECORE'; break;
//			case '9.7': $resultado_acao = 'DBE'; break;
//			case '9.9': $resultado_acao = 'Avulso'; break;
...
//		}
		...
		switch ($linha["resultado_acao"]){
			...
			case '7': $resultado_acao = 'Conta reativada'; break;								// não vai
			case '8': $resultado_acao = 'Demo a vencer'; break;							// não vai
			case '7.1': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;
			case '8.2': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.3': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.4': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.5': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.6': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;  
			case '8.7': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.8': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '8.9': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '9.0': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '9': $resultado_acao = 'Inconsistência boleto'; break; 
			case '9.1': $resultado_acao = 'Boleto valor menor'; break; 
			case '9.2': $resultado_acao = 'Boleto valor maior'; break; 
			case '9.3': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '9.4': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;
			case '9.5': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '9.6': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;  
			case '9.7': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 
			case '9.8': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break; 				
			case '9.9': $resultado_acao = 'Venda cancelada'; break; 
			case '10.1': $resultado_acao = 'Boleto com sucesso'; break;
			case '11.0': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;
			case '11.1': $resultado_acao = ($linha["tipo_cobranca"] == 'boleto' ? 'Boleto com sucesso' : 'Cartão com sucesso'); break;	
