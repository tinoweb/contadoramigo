...
//					printf("<br>".$sql."<BR>");
					...
					*/
					if($loop_pagamentos < mysql_num_rows($resultado)){
						...
						$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, data, tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "', '" . date('Y-m-d') . "', 'boleto', '9.1', '', " . $valor_pago . ".00)";
						//$RETORNO .= $sqlup . "<br>";
		//					printf("<br>".$sqlRelCobranca."<BR>");
						...
						if($loop_pagamentos > mysql_num_rows($resultado)){
							...
							//Mensagem a exibir no relatório final.
							// INSERINDO LINHA NO RELATORIO DE COBRANCA INFORMANDO ESTA INCONSISTENCIA
							$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, data, tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "', '" . date('Y-m-d') . "', 'boleto', '9.2', '', " . $valor_pago . ".00)";
							//$RETORNO .= $sqlup . "<br>";
			//					printf("<br>".$sqlRelCobranca."<BR>");
							$resultadoup = mysql_query($sqlRelCobranca)
							or die (mysql_error());
	...
	//						printf("<br>".$sqlup."<BR>");
							...
							$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, data, tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "', '" . date('Y-m-d') . "', 'boleto', '1.2', 'enviado', " . $valor_pago . ".00)";
							//$RETORNO .= $sqlup . "<br>";
			//					printf("<br>".$sqlRelCobranca."<BR>");
							$resultadoup = mysql_query($sqlRelCobranca)
							or die (mysql_error());
					...
					if(mysql_num_rows(mysql_query($sql_checamesboleto)) > 0){
						...
						$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, data, tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "', '" . date('Y-m-d') . "', 'boleto', '9.3', '', " . $valor_pago . ".00)";
						$resultadoup = mysql_query($sqlRelCobranca)
						or die (mysql_error());
...
//				printf("<br>".$sqlChecaVencidos."<BR>");
				...
				if(mysql_num_rows($resultadoChecaAVencer) <= 0 && mysql_num_rows($resultadoChecaVencidos) <= 0){
...
function afterSuccess(){
	//location.href='cobranca_upload.php';
//	alert("'"+$('#output').html()+"'");
//	if($('#output').html() == ''){
//		$('#output').append('<div style="clear:both;margin-top:10px;"><input type="button" id="btReload" value="Fechar"></div>');
...
<input type="button" value="Iniciar Cobrança" onClick="location.href='cobranca.php?iniciarCobranca&dataFim=<?=$dataFim?>&dataInicio=<?=$dataInicio?>'" />
...
// LOOP PARA CANCELAMENTO DE USUARIOS - PARA SEGUNDA - quanquer não pago há mais de 90 dias
	...
	*/
	...
	// ############################## LOG ###############################
	if($show_log == true){
		echo "checando se há pagamentos de boletos a vencer para enviar email cobrando (retornou " . mysql_num_rows($rsBoletos_a_vencer) . ")<BR><BR>";
	}
	// ############################## LOG ###############################
	// LOG DE ACESSOS
	...
	*/
	...
	// ############################## LOG ###############################
	if($show_log == true){
		echo "checando se há usuários demos a vencer email informando (retornou " . mysql_num_rows($rsDemos_a_vencer) . ")<BR><BR>";
	}
	// ############################## LOG ###############################
	// LOG DE ACESSOS
...
//			   	AND h.tipo_cobranca IN ('visa','mastercard')
	...
	*/
	...
	if($show_log == true){
		...
		echo "pegando os ids que não estão cancelados e os status de pagamento estão vencidos (retornou " . mysql_num_rows($resultado) . ")<BR><BR>";
	}
	// LOG DE ACESSOS
	...
	*/		
	...
	// ############################## LOG ###############################
	if($show_log == true){
		echo "início do loop para processar a cobrança<BR><BR>";
	}
	// ############################## LOG ###############################
...
//		printf("<br>".$sqlChecaAVencer."<BR>");
		...
		if($linha['status'] == 'demo'){
			...
			if(($forma_pagamento_assinante == 'visa') || ($forma_pagamento_assinante == 'mastercard')){
				if(($retorno_codigo_erro == '') && ($retorno_codigo_autorizacao != '5')){
					...
					if(mysql_num_rows($resultadoChecaAVencer) <= 0){// tirado em 21/01/2014 && mysql_num_rows($resultadoChecaVencidos) <= 0){
						...
						// ############################## LOG ###############################
						if($show_log == true){
							echo "criando pagamento a vencer para usuario demo na data " . ($dataPagamento) . "<BR><BR>";
						}
						// LOG DE ACESSOS
				...
				if(mysql_num_rows($resultadoChecaPago) > 0 && mysql_num_rows($resultadoChecaAVencer) <= 0){
			...
			//if($linha['status'] == 'demoInativo'){	
				...
				if(mysql_num_rows($resultadoChecaAVencer) <= 0){
					...
					// ############################## LOG ###############################
					if($show_log == true){
						echo "criando pagamento a vencer na data " . ($dataPagamento) . "<BR><BR>";
					}
					// LOG DE ACESSOS
	...
	*/		
	// ############################## LOG ###############################
	if($show_log == true){
		echo "Procedendo com a alteração de status de login dos usuários<BR><BR>";
	}
	// LOG DE ACESSOS
...
?>
<a class="imagemDica" div="divFormUpload" id="btCarregaArquivo" style="z-index: 99;position: relative; padding: 5px 5px 0 0;  float: right; margin-right: 82px; text-align:right; font-weight: normal; font-size: 11px; text-decoration: underline; color: #336699">Carregar arquivo de notas fiscais</a>
<div class="tituloVermelho">
	<?=$titulo_vermelho?>
</div>
<div style="clear: both;"></div>
<table border="0" cellspacing="2" cellpadding="4" style="font-size:11px">
    <tr>
    <th style="width:55px;" align="center">Data</th>
    <th style="width:215px;" align="center">Razão Social</th>
    <th style="width:115px;" align="center">CNPJ</th>
    <th style="width:75px;" align="center">TID</th>
    <th style="width:95px;" align="center">Nº cartão</th>
    <th style="width:55px;" align="center">Valor</th>
    <th style="width:125px;" align="center">Resultado da Ação</th>
    <th style="width:65px;" align="center">nº NF</th>
    </tr> 
<?php
while ($linha=mysql_fetch_array($resultado)) {
...
?>
    <tr class="guiaTabela" style="background-color:#<? 
		...
		<td><a href="cliente_administrar.php?id=<?=$linha["id"]?>" target="_blank">
		  <?=(strlen($linha2["razao_social"]) > 33 ? substr($linha2["razao_social"],0,30) . "..." : $linha2["razao_social"])?>
		</a></td>
...
<!--
		...
		?></td>
-->
        <td align="center" class="col_tid" id="idRel_<?=$linha["idRelatorio"]?>"><?=$linha['tid'] != '' ? str_replace("1032144332","",$linha['tid']) : '';?></td>
...
//alert(objCampo.val());
  ...
  ,montaCampoTexto: function(classe_objeto, value_campo) {
	  if(classe_objeto == 'col_nf'){
		  return '<input style="width:60px; text-align: center; font-size: 100%" type="text" name="numero_nf_cobranca" id="numero_nf_cobranca" value="' + value_campo + '">';
	  }
	  if(classe_objeto == 'col_tid'){
		  return '<input style="width:70px; text-align: center; font-size: 100%" type="text" name="numero_tid_cobranca" id="numero_tid_cobranca" value="' + value_campo + '">';
	  }
  }
});
