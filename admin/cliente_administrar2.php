...
function pegaDataPagamento($id,$idHistorico) {
	...
	if(mysql_num_rows($resultado) > 0){
		...
		$out = "<a class='pagtoRelacionamento' href='#' data-idHistorico='".$idHistorico."' data-idRelatorio='".$dadosPagto['idRelatorio']."'>".date('d/m/Y', strtotime($dadosPagto['data']))."</a>";

	} else {

		$out = "<a class='pagtoRelacionamento' href='#' data-idHistorico='".$idHistorico."' data-idRelatorio='Sem'>Sem Pagamento</a>"; 
	}
	
	$out .= "<div style='display:;'>
				<form method ='post'>
					<input type='hidden' name='idHistorico' value='$idHistorico' />
					<input type='hidden' name='idRelatorio' value='' />
					<select id='historico_".$idHistorico."' name='pagtoData'>
...
<script>
	...
	$( document ).ready(function() {
		...
		$('.pagtoRelacionamento').click(function(){
