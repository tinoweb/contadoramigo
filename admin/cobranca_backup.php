				...
				if( ( $status_boleto == 6 || $status_boleto == 17 ) ){
					...
					{				
						...
						if ( false )
						{
							...
							$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id,idHistorico , data, data_pagamento, tipo_cobranca, resultado_acao, envio_email, valor_pago, observacao) VALUES ('" . $id_user . "', '".$data_historico_aux."' ,'" . $data_hoje . "', '" . $data_hoje . "', 'boleto', '1.2', '', " . $valor_pago . ",'Boleto com valor diferente do plano')";
						...
						{		
							...
							if ($meses_a_somar > 0)
							{
								...
								$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, idHistorico , data, data_pagamento, tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "', '".$data_historico_aux."' , '" . $data_hoje . "', '" . $data_hoje . "', 'boleto', '1.2', 'enviado', " . $valor_pago . ")";
								$resultadoup = mysql_query($sqlRelCobranca)
								or die (mysql_error());
						...
						{					
							...
							*/
							...
							{						
								...
								$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, idHistorico ,data, data_pagamento, tipo_cobranca, resultado_acao, envio_email, valor_pago, observacao) VALUES ('" . $id_user . "','".$data_historico_aux."' ,'" . $data_hoje . "', '" . $data_hoje . "', 'boleto', '1.2', '', " . $valor_pago . ",'Boleto valor menor')";
								$resultadoup = mysql_query($sqlRelCobranca)
								or die (mysql_error());
							...
							{						
								if($loop_pagamentos > mysql_num_rows($resultado))
								{
									...
									//Mensagem a exibir no relatório final.
									// INSERINDO LINHA NO RELATORIO DE COBRANCA INFORMANDO ESTA INCONSISTENCIA
									$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id,idHistorico ,data, data_pagamento, tipo_cobranca, resultado_acao, envio_email, valor_pago, observacao) VALUES ('" . $id_user . "', '".$data_historico_aux."' ,'" . $data_hoje . "', '" . $data_hoje . "', 'boleto', '1.2', '', " . $valor_pago . ",'Boleto valor maior')";
									$resultadoup = mysql_query($sqlRelCobranca)
									or die (mysql_error());	
						...
						else
						{
							...
							if(mysql_num_rows($rs_checaMesPago) <= 0)
							{
								...
								else
								{
									...
									$sqlRelCobranca = "INSERT INTO relatorio_cobranca (id, idHistorico , data, data_pagamento,tipo_cobranca, resultado_acao, envio_email, valor_pago) VALUES ('" . $id_user . "','".$data_historico_aux."' , '" . $data_hoje . "','" . $data_hoje . "', 'boleto', '9.3', '', " . $valor_pago . ")";
									$resultadoup = mysql_query($sqlRelCobranca)
									or die (mysql_error());
...
if(isset($_GET["iniciarCobranca"])) { 
	...
	*/
	...
	//Átano - Foi implementado a verificação de dias uteis, ou sejá ele nao inclui como vencido se for sábado, domingo e feriados.
		$sqlUpdateVencidos = "
			UPDATE 
	...
	while ( $rodar ){
		...
		SET 
			h.status_pagamento = (CASE 
								...
								END) 
		WHERE 
			status_pagamento IN ('a vencer','pendente') 
			AND DATEDIFF(data_pagamento, DATE(now())) <= 0
		";
		$sqlUpdateNaoPago = "UPDATE `historico_cobranca` SET status_pagamento = 'não pago' WHERE status_pagamento IN ('vencido') AND DATEDIFF(data_pagamento, DATE(now())) <= -6";
		$rsAtualizaNaoPago = mysql_query($sqlUpdateNaoPago);
