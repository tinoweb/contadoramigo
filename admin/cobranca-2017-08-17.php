...
<div class="titulo" style="margin-bottom:10px; float:left">Pagamentos</div>
...
<div style="position: relative; font-size: 11px;margin-top:10px;">
    ...
    Exibir 
    <select name="selAcao" id="selAcao" style="font-size: 11px; width: 130px;"> 
    <?
    /*
...
//	$rs_checa_cancelamento = mysql_query($sql_checa_cancelamento);
	...
	// LOOP PARA CANCELAMENTO DE USUARIOS - PARA SEGUNDA - quanquer não pago há mais de 60 dias
	
	// CHECANDO NO HISTORICO DE COBRANCA SE HÁ REGISTRO NÃO PAGO HA MAIS DE 2 MESES PARA DETERMINAR SE DEVE CANCELAR O USUARIO
	$sql_checa_cancelamento = "
								SELECT 
									distinct h.id 
								FROM login l
								INNER JOIN 
									historico_cobranca h ON l.id = h.id
								WHERE 
									DATEDIFF(h.data_pagamento, DATE(NOW())) < -60 
									AND h.status_pagamento =  'não pago'
									AND l.status <> 'cancelado' 
							";
	$rs_checa_cancelamento = mysql_query($sql_checa_cancelamento);
						...
						*/	
						...
						if (($retorno_codigo_erro == '') && ($retorno_codigo_autorizacao != '5')) { //Código 5 equivale a transação não autorizada
							...
							if(is_null($retorno_tid) || $retorno_tid == ""){
								...
								$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.4', 'não enviado', '', " . $valor_pago . ", '" . $numeroCartao."','".$linha["tipo_plano"]."','".$linha["plano"]."', 'Registro linha: 1967')";
								$resultadoup = mysql_query($sqlup)
								or die (mysql_error());
								...
								// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
								$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.1', 'enviado', '".$retorno_tid."', " . $valor_pago . ", '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."', 'Registro linha: 2021')";
								$resultadoup = mysql_query($sqlup)
								or die (mysql_error());
						...
						//       Caso retorne com erro.          Caso o cartão ocorra uma transação não autorizada.
							...
							// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
							$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, numero_cartao, tipo_plano, plano, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '" . $acao_relatorio . "', 'enviado', '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."', 'Registro linha: 2101')";
							$resultadoup = mysql_query($sqlup)
							or die (mysql_error());
						...
						// INSERINDO O REGISTRO NO RELATÓRIO DE COBRANÇA PARA ESTE PAGAMENTO.
						$sqlup = "INSERT INTO relatorio_cobranca (id, idHistorico ,  data, tipo_cobranca, resultado_acao, envio_email, tid, valor_pago, numero_cartao, tipo_plano, plano, observacao) VALUES ('" . $linha["idUser"] . "','".$linha["idHistorico"]."' ,'" . $data_pagamento_cartao . "', '" . $forma_pagamento_assinante . "', '2.5', 'não enviado', '', " . $valor_pago . ", '" . $numeroCartao ."','".$linha["tipo_plano"]."','".$linha["plano"]."', 'Registro linha: 2131')";
						$resultadoup = mysql_query($sqlup)
						or die (mysql_error());
...
//		} else {
			...
			else if(($linha["resultado_acao"] == "8.2" || $linha["resultado_acao"] == "8.3" || $linha["resultado_acao"] == "8.4" || $linha["resultado_acao"] == "8.5" || $linha["resultado_acao"] == "8.6" || $linha["resultado_acao"] == "8.7" || $linha["resultado_acao"] == "8.8" || $linha["resultado_acao"] == "8.9" || $linha["resultado_acao"] == "9.0" || $linha["resultado_acao"] == "9.4" || $linha["resultado_acao"] == "9.5" || $linha["resultado_acao"] == "9.6" || $linha["resultado_acao"] == "9.7") && $linha['emissao_NF'] == 0){
				echo "fdfddd";
			}  
...
-->
       ...
       <td><?php
		...
		switch ($linha["resultado_acao"]){
			...
			case '9.5': $resultado_acao = 'AberturaSociedade'; break;
