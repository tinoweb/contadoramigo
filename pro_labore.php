...
<div class="tituloVermelho" style="float: left">Pró-Labore</div>

<div class="tituloAzul" style="float: right"> 
<a href="#" class="btReAbrirBox" div="video" style="text-decoration: none"><i class="fa fa-file-video-o" aria-hidden="true"></i> Vídeo de orientações</a></div>
<div style="clear:both;"></div>
...
<div style="margin-bottom:20px">Pró-labore são as retiradas mensais do sócio/proprietário, para seu próprio sustento. Funciona como um salário e, assim, está sujeito a retenção de INSS e IR. Para não pagar muito imposto, costuma-se estipular um valor baixo e complementar a renda por meio da <a href="https://www.contadoramigo.com.br/distribuicao_de_lucros.php">distribuição de lucros</a>, que é isenta de impostos até um determinado limite.<br><br>

Utilize esta página para emitir seus recibos de pró-labore, calcular o recolhimento ao INSS e eventuais retenções de Imposto de Renda Pessoa Física. <strong>A empresa deverá depositar na sua conta pessoa física o valor líquido do pró-labore</strong>, demonstrado no recibo.<br /><br>

 O valor de seu pró-labore pode ser estipulado livremente, porém tenha em mente os seguintes pontos:
  <ul>
  <li>O valor médio do pró-labore será tomado como base pela Previdência para o cálculo de sua aposentadoria</li>
  <li> O recolhimento ao INSS corresponderá a 11% do pró-labore declarado</li>
  <li> O pró-labore não pode ser inferior a 1 salário mínimo <strong>(R$<?=$Salario_Minimo?>)</strong>.</li>
  <li> Se o pró-labore for superior ao teto previdenciário <strong>(R$<?=$Teto_Previdenciario?>)</strong>, o valor do recolhimento do INSS permanece igual a <strong><?=$Contribuicao_Maxima?></strong>.</li>
  <li>Se o valor líquido do pró-labore (descontado o INSS) for superior a <strong>R$ <?= $Limite_Insencao ?></strong>, você precisará fazer a retenção de Imposto de Renda.</li>
	  <li> Se a empresa não estiver faturando, você não precisa fazer retiradas de pró-labore.</li>
	  <li> Mesmo aposentado, você precisará fazer o recolhimento do INSS ao retirar pró-labore.</li>
  </ul>
</div>
...
<?
$query = mysql_query('SELECT idSocio, nome, dependentes, perc_pensao FROM dados_do_responsavel WHERE id = '.$_SESSION["id_empresaSecao"].' ORDER BY nome');
...
<label for="RetencaoINSS" style="margin-right: 10px">INSS:</label> <input name="RetencaoINSS" id="RetencaoINSS" type="text" size="21" maxlength="50"  readonly="readonly" />
<div style="clear:both; height:5px"></div>
<label for="RetencaoIR" style="margin-right: 24px">IR:</label> <input name="RetencaoIR" id="RetencaoIR" type="text" size="21" maxlength="50"  readonly="readonly" />
<div style="clear:both; height:5px"></div>
<label for="ValorLiquido" style="margin-right: 10px">Valor líquido a ser pago: <strong>R$</strong></label> <input name="ValorLiquido" id="ValorLiquido" type="text" size="21" maxlength="50" style="font-weight:bold" readonly />
...
<div style="position: relative;">
  ...
  <li>O recolhimento do <strong>IR</strong>, se houver, deverá ser efetuado por meio do DARF até o dia 20 do mês seguinte aos pagamentos. O calendário fiscal do Contador Amigo o lembrará deste pagamento e <a href="darf_orientacoes.php">o tutorial</a> o guiará na geração da guia.</li>
  </ul>
...
<?
	...
	function MontagemDaListagemDosPagamentos($dataInicio, $dataFim, $idNome){
		...
		$resOrdem = "
				...
				";		
		//return $sql.$resDatas.$resColuna.$resOrdem;
 	...
 	function PegaUltimoPagamento(){
		...
		$sql2 = "SELECT * FROM dados_pagamentos WHERE id_login = '" . $_SESSION["id_empresaSecao"] . "' AND id_socio != 0 ORDER BY data_pagto DESC LIMIT 1";
	...
	?>
                    <tr>
                        <td class="td_calendario" align="center">
                            ...
                            <a href="Recibo_download.php?id_pagto=<?=$idPagto?>" title="Baixar PDF">
...
<!--                        <td class="td_calendario" align="right"><?=number_format($desc_dep,2,',','.')?></td>-->
