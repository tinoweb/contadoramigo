...
//	error_reporting(E_ALL);
	...
	class Boleto{
		...
		private $AlteracaoQtdeEmpresaBoleto;
		
		function getTiposServico() {
			return array('A_empresa','A_sociedade','decore','DBE','Gfip_GPS','Simples_DAS','Defis','Rais_negativa','Dirf','F_empresa','F_sociedade','MEI-ME','CPOM','regularizacao_emp','funcionario_C_D','servico_geral','DCTF','DeSTDA','IRPF');
		}
		function getTiposBoleto() {
			return array('A_empresa','A_sociedade','decore','DBE','Gfip_GPS','Simples_DAS','Defis','Rais_negativa','Dirf','F_empresa','F_sociedade','MEI-ME','CPOM','regularizacao_emp','funcionario_C_D','servico_geral','ComplementarPremium','DCTF','DeSTDA','IRPF');
		}
		function getvia(){
			return $this->via;
		}
		...
		function getAlteracaoQtdeEmpresaBoleto(){
			return $this->AlteracaoQtdeEmpresaBoleto;
		}
		function setAlteracaoQtdeEmpresaBoleto($valor){
			$this->AlteracaoQtdeEmpresaBoleto = $valor;
		}
		...
		function calcularVencimento(){
			...
			if( $objeto_consulta['status'] == 'ativo' && $this->getdata_vencimento() >= date("Y-m-d") ) {
				
				// Caso o dia do vencimento não seja um dia util e o prazo do boleto for 0 inclui 
				if( !$datas->ifDiaUtil($this->getdata_vencimento()) && $this->getprazo() == 0 ) {
					$this->setprazo(1);
				}
				...
				// Caso o dia do vencimento não seja um dia util e o prazo do boleto for 0 inclui 
				if( !$datas->ifDiaUtil(date("Y-m-d")) && $this->getprazo() == 0 ) {
					$this->setprazo(1);
				}
		...
		function getDadosUser(){
			...
			$this->setDescontoMesalidade($cobranca['desconto_mesalidade']);

			// Pega os valores de desconto.
			$this->setDescontoStandardMensal($cobranca['desconto_S_mensal']);
			$this->setDescontoStandardTrimestral($cobranca['desconto_S_trimestral']);
			$this->setDescontoStandardSemestral($cobranca['desconto_S_semestral']);
			$this->setDescontoStandardAnual($cobranca['desconto_S_anual']);
			$this->setDescontoPremiumMensal($cobranca['desconto_P_mensal']);
			$this->setDescontoPremiumTrimestal($cobranca['desconto_P_trimestral']);
			$this->setDescontoPremiumSemestral($cobranca['desconto_P_semestral']);
			$this->setDescontoPremiumAnual($cobranca['desconto_P_anual']);
			
			// Verifica se o boleto e um boleto avulso complementar para o premium.
			if( $this->gettipo_boleto() == 'ComplementarPremium' ) {
				$this->setTipoPlano('P');
			} else {
			...
			if( $this->gettipo_boleto() == 'mensalidade' ) {
				...
				// pega o codigo do contador no banco
				$this->setContadorId($cobranca['contadorId']);
			}			
		...
		function definirMulta(){
			...
			// Pega a data de vencimento original.
			$vencimento_original = $this->getvencimento_original();
			
			// Verifica se a data horiginal e um dia util. 
			while(!$datas->ifDiaUtil($vencimento_original)) {
				$vencimento_original  = $datas->somarData($vencimento_original,1);
			}
		...
		function gerarMulta(){
			...
			//calcula juros.
			$multa = (($this->getvalor_boleto() * 2)/100);
		...
		//Define se havera juros de mora de acordo com a data que esta sendo gerado o boleto, caso seja maior que a data de vencimento original, gera jurosde mora na forma 0.22 * $dias
		...
		function getValorPlano($tipoPlano = 'S'){
			...
			$valorPlano = $objeto['valor'];
			
			// Verifica se deve ser aplicado o desconto do plano de assinatura.
			if($this->getDescontoMesalidade() == '1') {
				$valorPlano = $valorPlano - $objeto['valor_desconto'];
				
			}// Verifica se deve ser aplicado o desconto do plano de assinatura.
			elseif($this->getDescontoMesalidade() == '2') {
				
				// Verifica o tipo do plano para aplica o desconto.
				switch($this->getplano_user()){
					case 'mensalidade':
						if($tipoPlano == 'P') {
							$valorPlano = $valorPlano - $this->getDescontoPremiumMensal();
						} else {
							$valorPlano = $valorPlano - $this->getDescontoStandardMensal();
						}
						break;
					case 'trimestral':  
						if($tipoPlano == 'P') {
							$valorPlano = $valorPlano - $this->getDescontoPremiumTrimestal();
						} else {
							$valorPlano = $valorPlano - $this->getDescontoStandardTrimestral();
						}	
						break;

					case 'semestral':   
						if($tipoPlano == 'P') {
							$valorPlano = $valorPlano - $this->getDescontoPremiumSemestral();
						} else {
							$valorPlano = $valorPlano - $this->getDescontoStandardSemestral();
						}
						break;
					case 'anual':
						if($tipoPlano == 'P') {
							$valorPlano = $valorPlano - $this->getDescontoPremiumAnual();
						} else {
							$valorPlano = $valorPlano - $this->getDescontoStandardAnual();
						}
						break;
				}
			}
		...
		function VerificaRemessaAceita(){

			$out = 0;
			
			$consulta = mysql_query("SELECT remessa_aceita FROM boletos_registrados WHERE nosso_numero = '".$this->getconvenio().$this->getnosso_numero()."' ");
			$objeto_consulta = mysql_fetch_array($consulta);
			
			if(isset($objeto_consulta['remessa_aceita'])) {
				$out = $objeto_consulta['remessa_aceita'];
			}
		...
		function getGerarNossoNumeroBoletoMensalidade(){
			$this->definirVia();
			return str_pad($this->getvia().str_pad( $this->getid_historico() , 5 , "0" , STR_PAD_LEFT ), 10 , "0" , STR_PAD_RIGHT );
		}
		...
		function getGerarNossoNumeroBoletoAvulso(){
			...
			return  str_pad('9'.str_pad( $this->getMaxIdBoletosAvulsos() , 5 , "0" , STR_PAD_LEFT ), 10 , "0" , STR_PAD_RIGHT );
		}
		...
		function getGerarNossoNumeroBoleto(){
			//Formato: 9 + id_boletos_avulsos
			return  str_pad('9'.str_pad( $this->getMaxIdBoletos() , 5 , "0" , STR_PAD_LEFT ), 10 , "0" , STR_PAD_RIGHT );
		}
		...
		function getGerarNossoNumeroBoletoCertificado(){
			if( !$this->ifsegundaViaCertificado() )
				return  str_pad('8'.str_pad( $this->getMaxIdBoletosCertificados() , 5 , "0" , STR_PAD_LEFT ), 10 , "0" , STR_PAD_RIGHT );
			else 
		...
		function gerarNossoNumero(){
			...
			else if( in_array($this->gettipo_boleto(), $this->getTiposBoleto()) ){ 
		...
		function verificaSegundaVia(){
			...
			if( isset($objeto['numdoc_get']) && $objeto['numdoc_get'] == $this->getnosso_numero() ) {
				...
				// Se o boleto existir, mas se o cliete realizou o cancelamento da empresa ou abril uma empresa
				if($this->VerificaAlteracaoQuantidadeDeEmpresa()) {
				
					// query utilizada para verificar quandos boletos existe para poder gerar um novo sequencial do nosso numero.
					$consulta = mysql_query("SELECT * FROM boletos_registrados WHERE 
												id_historico = '".$this->getid_historico()."' AND
												id_user = '".$this->getid_user()."'");
					...
					// Gera os primeiro seis numeros. 					
					$numeroaux = $this->getvia().str_pad( $this->getid_historico() , 5 , "0" , STR_PAD_LEFT );
					
					// Pega o nosso numero com uma nova sequencia.
					$nosso_numero = str_pad( $numeroaux , 10 , "0" , STR_PAD_RIGHT );
					
					// Informa o nosso numero novo.
					$this->setnosso_numero($nosso_numero);
					$this->setnumero_documento($nosso_numero);
				...
				else if( intval($objeto['valor_get']) != intval($this->getvalor_boleto()) && $objeto['remessa_gerada'] == 0 ){
					
					mysql_query("DELETE FROM boleto WHERE id = '".$objeto['id']."' ");
					
				} // Verifica se o boleto esta vencido caso ele esteja define uma nova sequencia 
				elseif(isset($objeto['vencto_get']) && !empty($objeto['vencto_get']) && $objeto['vencto_get'] < date('Y-m-d') && $this->VerificaRemessaAceita() == 0){
					...
					// Verifica se a data do vencimento do boleto cai em dia util - 18/12/2017.
					$getdata_vencimento = $this->getdata_vencimento();
					
					// Verifica se e dia util - data de inclusão do código 18/12/2017.  
					if(!$datas->ifDiaUtil($getdata_vencimento)) {
						$getdata_vencimento = $datas->somarDiasUteis($getdata_vencimento, 1);
					}
										
					//Se o boleto ja venceu, seta a variavel para definir o recalculo do boleto atraves do portal do BB com um iframe
					if( $getdata_vencimento < date("Y-m-d") && $objeto['remessa_gerada'] == 1 ) {
...
//						$this->setisBoletoVencido(true);
		...
		// Método criado para verificar se houve alteração da quantidade empresas com relação a registrada no boleto.
		function VerificaAlteracaoQuantidadeDeEmpresa(){
			
			$out = false;
			
			// Verifica se o boleto gerado com remessa aceita esta no mesmo plano do boleto atual.
			$boleto = mysql_query("SELECT * FROM boletos_registrados WHERE nosso_numero LIKE '%".$this->getnosso_numero()."' ");	
			$reg = mysql_fetch_array($boleto);
			
			// Verifica se o plano e o mesmo do boleto. 
			if($this->getTipoPlano() == $reg['tipo_plano'] && $this->getplano_user() == $reg['plano']) {
				
				// Verifica se a quantidade de empresa foi alterado.
				if($this->gettotal_empresas() != $reg['numero_empresa']){
					$out = true;
				}
			}
			
			// Retorna a informação se teve alteração da quantidade de empresa.
			return $out;
		}
		...
		function registrarBoleto(){	
			$campos = "(`id_user`, `id_historico`, `nosso_numero`, `status`, `tipo`, Tipo_plano,`plano`, `valor`, `multa`, `mora`, `vencimento`, `numero_empresa`)";
			$values = "(
				'".$this->getid_user()."',
			...
			if($this->getContadorId() && in_array($this->gettipo_boleto(), $this->getTiposServico())) {
					
				// Formata o valor Total.	
				$valorTotal = $this->getvalor_boleto();
				
				// Pega o valor liquido e inclui o valor pago ou apagar.
				$valorLiquido = $this->getvalor_boleto() - number_format(($this->getvalor_boleto() / 2),2,'.','');
								
				mysql_query("INSERT INTO `cobranca_contador` (`contadorId`,`boletosRegistradosId`,`valor_total`,`valor_liquido`) VALUE (".$this->getContadorId().", ".$boletosRegistradosId.", ".$valorTotal.", ".$valorLiquido." );");
				
				$cobrancaContadorId = mysql_insert_id();
			
				// Inclui o serviço do boleto.
				mysql_query("INSERT INTO servico_avulso (id_user, contadorId, servico_name, data, valor, cobrancaContadorId, tipoPagamento) VALUES ('".$this->getid_user()."', '".$this->getContadorId()."', '".$this->gettipo_boleto()."', NOW(), '".$this->getvalor_boleto()."','".$cobrancaContadorId."', 'boleto')");
				
			} else if($this->getContadorId() && $this->getTipoPlano() == 'P' && $this->gettipo_boleto() == 'ComplementarPremium') {
				...
				mysql_query("INSERT INTO `cobranca_contador` (`contadorId`,`boletosRegistradosId`,`valor_total`,`valor_liquido`,`desconto_porcentagem`
) VALUE (".$this->getContadorId().", ".$boletosRegistradosId.", ".$valorTotal.", ".$valorLiquido.", ".$desconto.");");
				...
				$valorTotal = $this->getvalor_boleto() - number_format(($this->getvalor_boleto() / 2),2,'.','');
		...
		function definirValorBoleto(){
			...
			// Pega o valor multiplicado por empresa. 
		...
		function verificaStatusUsuario(){
			...
			if( $objeto_consulta['status'] == 'demoInativo' || $objeto_consulta['status'] == 'cancelado' || isset($_SESSION['AtivacaoContaDesativada'])){ 
				...
				// Caso o dia de hoje não seja um dia util ele prega o proximo dia util.
				if( !$datas->ifDiaUtilSexta(date("Y-m-d"))) {
					$this->setdata_vencimento($datas->somarDiasUteisExcetoSexta(date("Y-m-d"), 1));
				} else {
					$this->setdata_vencimento(date("Y-m-d"));
				}
			}
		}
		...
		function DefinePadraoBoletoAvulso($tipo_boleto, $dia = 0) {
			
			//Cria um objeto para manipular datas
			$datas = new Datas();
			//Define o tipo de boleto
			$this->settipo_boleto($tipo_boleto);
			//Define o valor do boleto
			$this->setvalor_boleto(str_replace(',', '.' , str_replace('.', '' , $_GET['valor'])));
			//Define o vencimento original do boleto
			$this->setdata_vencimento($datas->converterData($_GET['data']));
			//Define o id do uusuario para pegar os dados
			$this->setid_user($_GET['id_user']);
			//define o prazo em dias para o vencimento a partir de hoje.
			$this->setprazo($dia);
			//Pega o codigo do contador.
			$this->setContadorId($_GET['contadorId']);
			//Gera o boleto
			$this->gerarBoleto();
	...
	if( $_GET['tipo'] == "mensalidade" ){
	...
	else if( in_array($_GET['tipo'], $boleto->getTiposBoleto())){
		...
		//Define o tipo de boleto avulso
		$boleto->DefinePadraoBoletoAvulso($_GET['tipo'], $dia);
	}
?>