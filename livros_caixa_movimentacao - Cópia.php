...
//	error_reporting(E_ALL);
	...
	$controller = new LivrosCaixaMovimentacao();
...
<script>
	...
	$(document).ready(function(e) {
		...
		function DesabilitaCamposComplementares(){
			...
			$("#servicosPrestados").css("display","none");
...
<div class="titulo" style="float: left">Livro Caixa</div>
...
//echo $descricao_colunas_tabela['Field'];
				...
				<div style="margin-bottom:20px">
					...
					<span style="margin-right: 5px;">Entrada/Saída:</span>				
					...
					<span style="margin-left: 10px; margin-right: 5px;">Categoria:</span>
					<select name="selCategoria" id="selCategoria" style="width:190px;font-size:12px !important;">
						<option value="">selecione</option>
					</select>
					
					<span style="margin-left: 10px; margin-right: 5px;">Novo:</span>
					<select name="selemprestimo">
						<option value="nao">Não</option>
						<option value="sim">Sim</option>
					</select>					
					
					<span style="margin-left: 10px; margin-right: 5px;">Existe Lançamento da Nota:</span>
					<select name="selContasAReceberNota">
						<option value="sim">Sim</option>
						<option value="nao">Não</option>
					</select>
...
<!--									<select name="selCategoria" id="selCategoria" style="width:190px;font-size:12px !important;">
						...
						<table border="0" cellspacing="2" cellpadding="4" style="width: 100%;margin-bottom: 20px;margin-top: -2px">
							<tbody>
								<tr id="opcao_emprestimo" status="fechado" style="display:none; background-color: #e5e5e5;">
									<td class="td_calendario" style="font-size: 12px;">
										<?php 
											// Realiza a consulta dos emprestimos.
											$qryEmprestimo = mysql_query("SELECT * FROM dados_do_emprestimo WHERE empresaId = '".$_SESSION['id_empresaSecao']."' AND apelido IS NOT NULL AND tipo = 'emprestimo'");
					
											// Verifica se existe empréstimos que ainda não teve armotização e cria a lista.
											if(mysql_num_rows($qryEmprestimo) > 0):
											
												$optionEmprestimo = '';

												while($array = mysql_fetch_array($qryEmprestimo)){
													$optionEmprestimo .= "<option value=".$array['emprestimoId'].">".$array['apelido']."</option>";
												}
										?>
											...
											<div id="selecioneEmprestimo" style="display: inline-block;">
												...
												<select id="registroPaiId" name="registroPaiId" style="margin-left:5px; width:150px;">
													<option value=''>Selecione</option>
													<?php echo $optionEmprestimo;?>
												</select>
												<a id="mostraImput" href="">Novo apelido</a>
											</div>
											<div id="novoEmprestimo" style="display: none;">
												<span>Atribua um apelido para este empréstimo:</span>
												<input id="apelido" name="apelido" type="text" value="" style="margin-left:5px; width:150px;" disabled="disabled" />
												<a id="mostraSelecione" href="">Voltar</a>
											
												<span style="margin-left: 30px;">Informe o prazo de carência do empréstimo:</span>
												<select id="prazo_carencia" name="prazo_carencia" style="margin-left:5px;width:40px">
													<option value=""></option>
													<?php 
														for ($i=1; $i < 25; $i++) { 
										...
										<?php else:?>
											...
											<span>Atribua um apelido para este empréstimo:</span>
											<input id="apelido" name="apelido" type="text" value="" style="margin-left:5px;" />
											<span style="margin-left: 30px;">Informe o prazo de carência do empréstimo:</span>
											<select id="prazo_carencia" name="prazo_carencia" style="margin-left:5px;width:40px">
												<option value=""></option>
												<?php 
								...
								<tr id="saldoDevedorRemanescente" style="display:none; background-color: #e5e5e5;">
									<td class="td_calendario" style="font-size: 12px;">
										...
										?>
										<span>Informe o empréstimo:</span>
										
										<select id="registroPaiIdAmortizacao" name="registroPaiIdAmortizacao" style="margin-left:5px;">
											<option value="">Selecione</option>
										 	<?php echo $optionEmprestimo;?>
										</select>
										 
										<span style="margin-left: 30px;">Saldo devedor remanescente:<span>
										<input id="saldoRemanescente" class="current" name="saldoRemanescente" type="text" value="" style="margin-left:5px;" />	
									</td>
								</tr>
								<tr id="servicosPrestados" style=" display: none; background-color: #e5e5e5;">
									<td class="td_calendario" style="font-size: 12px;">
										<span>Data da nota fiscal:</span>
										<input name="dataNotaFiscal" id="dataNotaFiscal" type="text" maxlength="10" style="width:80px;font-size:12px;" class="campoData" disabled />
									<td>
								</tr>
								<tr style="width: 100%;">
									<td class="td_calendario"  class="td_calendario" style="padding-bottom: 8px;padding-top:8px;">
							...
							<table border="0" cellspacing="2" cellpadding="4" width="966" style="font-size: 11px;">
								...
								<tbody>
									<tr>
										...
										<td class="td_calendario">
											<input name="txtData" id="txtData" type="text" value="<?=date('d/m/Y', strtotime($linha["data"]))?>" maxlength="10" style="width:70px;font-size:12px;" class="campoData" />
										</td>
									...
									//	echo str_replace(".",",",$linha["entrada"]);
											...
											?>" style="width:100px;font-size:12px !important;" class="current" />
										</td>										   
								...
								<?php 
									// Pega a tabela que fica com os dados para poder manipular os dados do empréstimo.
									echo $controller->LinhaDadosComplementaresEmprestimo($_SESSION['id_empresaSecao'], $linha['id']);
								?>
								
								<table border="0" cellspacing="2" cellpadding="4"  style="width: 100%;margin-bottom: 20px;margin-top: -4px;">
									<tbody>
										<tr style="width: 100%;">
...
<script>
	...
	$(function(){
		$('#mostraImput').click(function(e){
			e.preventDefault();
			$('#novoEmprestimo').css('display', 'inline-block');
		...
		$('#mostraSelecione').click(function(e){
			e.preventDefault();			
			$('#novoEmprestimo').hide();
			$('#selecioneEmprestimo').css('display', 'inline-block');
...
<script>
	...
	$( document ).ready(function() {
		...
		$("#selCategoria").change(function() {
			...
			verificarServicosPrestadosGeral(valor);
		...
		//Verifica se é Categoria Entrada->emprestimos para exibir o input com o przo de carencia    
		function verificarServicosPrestadosGeral(valor){
...
?>
			...
			if( (valor === 'Serviços prestados em geral' || status != -1 ) && $("#selTipoLancamento").val() === 'entrada' ){
				$("#servicosPrestados").css("display","block");
...
<?php 
	if(isset($_SESSION['erroEmprestimo'])){
		...
		if($_SESSION['erroEmprestimo'] == 'excluir'){
