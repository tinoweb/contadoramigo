...
<div class="tituloVermelho" style="float: left; margin-bottom:10px">Distribuição de Lucros</div>
...
<!-- style="border-style:solid; border-width:1px; width:680px; border-color:#CCCCCC; position:relative; left:50%; margin-left:-340px; background-color:#fff">-->
    <div style="padding:20px">
       ...
       <video id="video1" width="640" height="360" controls>
        ...
        <source src="videos/distribuicao_lucros.ogv" type='video/ogg'>
        ...
        <param name="flashvars" value="file=videos/distribuicao_lucros.mp4" />
        </object>
        </video>
    </div>
</div>
 ...
 <div style="clear:both;"></div>
  ...
  <br />
    Existem duas formas dos sócios  usufruirem da renda de suas empresas. A primeira é através do pró-labore, uma retirada mensal em valores constantes, a título de remuneração pelo trabalho desempenhado. O pró-labore é uma renda tributável, ou seja, você terá que recolher um percentual  para o IR e para a previdência. Para estar isento do IR, sua retirada líquida mensal de pró-labore não pode ser superior a R$ <?= $Limite_Insencao ?>
    ao mês. Já para o recolhimento à Previdência, não há isenção. Você deverá recolher o equivalente a 11% do pró-labore.<br />
  <br />
    A outra forma de retirar dinheiro da empresa é pela distribuição de lucros. A vantagem é que não há cobrança de IR, nem recolhimento à previdência. Normalmente a distribuição de lucros é feita no final do ano,
    após contabilizado o lucro obtido no período, mas você pode fazer antecipações mensais. <br />
    <br />
...
<span id="linha_periodo">
    ...
    <div style="display:none" id="div_mensal">
      <input name="periodo" id="mensal" type="text" style="width:80px;" class="campoDataMesAno" maxlength="7" /> (mm/aaaa)
    </div>
    <div style="display:none" id="div_anual">
      <input name="periodo" id="anual" type="text" style="width:60px;" class="campoDataAno" maxlength="4" /> (aaaa)
    </div> 
    <label for="dtPagto" style="margin-left:50px">Data do pagamento:</label>
    <input name="dtPagto" id="dtPagto" type="text" style="width:80px;" class="campoData" maxlength="10" /> (dd/mm/aaaa)
...
while($dados = mysql_fetch_array($query)){
	echo "	<!--<div style=\"position:relative;float:left;padding:5px;width:20px;\"><input type=\"checkbox\" name=\"idSocio[]\" value=\"".$dados['idSocio']."\"></div>" . "\r\n" . "-->
					...
					<div style=\"position:relative;float:left;padding:0px;width:140px;\">R$ <input type=\"text\" name=\"valorSocio\" style=\"width:100px;\" class=\"current\" maxlength=\"14\" /></div>" . "\r\n" . "
...
//		}
		...
		if(valor == ''){
			alert('É necessário preencher o valor destinado a este sócio.');
...
//					$data += "&idPagto=" + retorno;
					...
					$('#btSIMAvisoLivroCaixa').attr('valor',valor);
...
<!--                            <a href="#" onClick="if (confirm('Você tem certeza que deseja excluir este Pagamento?'))location.href='folha_pagamentos_excluir.php?excluir=<?=$idPagto?>;"><img src="images/del.png" width="24" height="23" border="0" title="Excluir" /></a>-->
                            ...
                            <a href="Recibo_distribuicao_download.php?id_pagto=<?=$idPagto?>" title="Salvar"><i class="fa fa-floppy-o iconesAzul iconesGrd"></i></a>
                        </td>
