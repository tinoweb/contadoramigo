...
*/
  public static function numero($valor, $genero = self::GENERO_MASC) {
    ...
    if(!is_numeric($valor))
      ...
      throw new Exception("[Exceção em GExtenso::numero] Parâmetro \$valor menor que zero (recebido: '$valor')");
...
//      $ret = self::numero($milhar, self::GENERO_MASC) . ' ' . self::$MILHAR; // 'Mil' é do gênero masculino
			$ret = self::numero($milhar, $genero) . ' ' . self::$MILHAR; // 'Mil' é do gênero masculino
      $resto = $valor % 1000;
      /* Chamada recursiva à função para processar $resto se este for maior que zero.
...
//      $ret = self::numero($milhoes, self::GENERO_MASC) . ' '; // Milhão e milhões são do gênero masculino
      $ret = self::numero($milhoes, $genero) . ' '; // Milhão e milhões são do gênero masculino
...
*/
  public static function moeda(
    ...
    if(!is_numeric($valor))
      ...
      throw new Exception("[Exceção em GExtenso::moeda] Parâmetro \$valor menor que zero (recebido: '$valor')");
